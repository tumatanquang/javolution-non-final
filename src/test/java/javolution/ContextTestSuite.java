...
public final class ContextTestSuite extends TestSuite {
	...
	class Concurrency extends TestCase {
		...
		public void validate() {
			...
			for(int i = 0; i < _size - 1; ++i) {
				final int i1 = ((Index) _table.get(i)).intValue();
				final int i2 = ((Index) _table.get(i + 1)).intValue();
			...
			if(size < 100) {
				...
				for(int i = 0, i1 = 0, i2 = 0; i < size; i++) {
					if(i1 >= t1.size()) {
						...
						final Index o1 = (Index) t1.get(i1);
						final Index o2 = (Index) t2.get(i2);
	...
	class SmallObjectAllocation extends TestCase {
		...
		public String getName() {
			return (!_useStack ? "HeapContext (default)" : "StackContext") + ", small object creation";
		}
		public void setUp() {
			c0 = new XYZ(0, 0, 0);
			c1 = new XYZ(-1, 1, 0);
	...
	class ArrayRecycling extends TestCase {
		...
		public String getName() {
			return "HeapContext, char[" + _size + "] " + (_recycle ? "recycled" : "allocated");
		}
		public void execute() {
			if(_recycle) {
	...
	private static final class XYZ {
		...
		public static XYZ valueOf(long x, long y, long z) {
			final XYZ c = (XYZ) FACTORY.object();
			c.x = x;
			c.y = y;
			c.z = z;
			return c;
		...
		public XYZ plusStack(XYZ that) {
			return XYZ.valueOf(x + that.x, y + that.y, z + that.z);
		}
		public XYZ plusHeap(XYZ that) {
			return new XYZ(x + that.x, y + that.y, z + that.z);
		}
	}
}