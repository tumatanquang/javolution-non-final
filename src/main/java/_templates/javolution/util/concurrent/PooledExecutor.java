 ...
 * remove       ThreadGroup-basedmethods
 ...
 * new          DiscardOldestWhenBlockedpolicy
...
public class PooledExecutor extends ThreadFactoryUser implements Executor {
	 ...
	 */
	private int _maximumPoolSize = DEFAULT_MAXIMUMPOOLSIZE;
	/**
	 * The minumum number of threads to maintain in pool.
	 */
	private int _minimumPoolSize = DEFAULT_MINIMUMPOOLSIZE;
	/**
	 * Current pool size.
	 */
	 ...
	 */
	private long _keepAliveTime = DEFAULT_KEEPALIVETIME;
	/**
	 ...
	 */
	private final Channel _handOff;
	/**
	 ...
	 */
	private final Map _threads;
	/**
	 * The current handler for unserviceable requests.
	 */
	private BlockedExecutionHandler _blockedExecutionHandler;
	/**
	...
	public synchronized void interruptAll() {
		for(final Iterator it = _threads.values().iterator(); it.hasNext();) {
	...
	public synchronized void shutdownNow(BlockedExecutionHandler handler) {
		...
		_shutdown = true; // don't allow new tasks
		_minimumPoolSize = _maximumPoolSize = 0; // don't make new threads
	...
	public synchronized void shutdownAfterProcessingCurrentlyQueuedTasks(
		...
		_shutdown = true;
		if(_poolSize == 0) {
			_minimumPoolSize = _maximumPoolSize = 0;
		}
	}
	/**
	...
	public List drain() {
		...
		final Vector tasks = new Vector();
		for(;;) {
			try {
				final Object x = _handOff.poll(0);
				if(x == null) {
					break;
				}
				tasks.addElement(x);
	 ...
	 */
	private synchronized void workerDone(Worker w) {
		_threads.remove(w);
		if(--_poolSize == 0 && _shutdown) {
			_maximumPoolSize = _minimumPoolSize = 0; // disable new threads
			notifyAll(); // notify awaitTerminationAfterShutdown
		}
		// Create a replacement if needed
		if(_poolSize == 0 || _poolSize < _minimumPoolSize) {
			try {
				final Runnable r = (Runnable) _handOff.poll(0);
				if(r != null && !_shutdown) {
					addThread(r);
				}
			}
	...
	private Runnable getTask() throws InterruptedException {
		long waitTime;
		synchronized(this) {
			if(_poolSize > _maximumPoolSize) // Cause to die if too many threads
				return null;
			waitTime = _shutdown ? 0 : _keepAliveTime;
		}
		if(waitTime >= 0)
			return (Runnable) _handOff.poll(waitTime);
		return (Runnable) _handOff.take();
	}
	/**
	 * Class defining the basic run loop for pooled threads.
	 */
	private final class Worker implements Runnable {
		private Runnable _firstTask;
		private Worker(Runnable firstTask) {
			_firstTask = firstTask;
		}
		public void run() {
			try {
				Runnable task = _firstTask;
				_firstTask = null; // enable GC
				if(task != null) {
					task.run();
					task = null;
				}
	 ...
	 * current _blockedExectionHandler.
	 */
	 ...
	 */
	private static final class RunWhenBlocked
	 ...
	 */
	private final class WaitWhenBlocked implements BlockedExecutionHandler {
		public boolean blockedAction(Runnable command)
				throws InterruptedException {
	 ...
	 */
	private static final class DiscardWhenBlocked
			implements BlockedExecutionHandler {
		public boolean blockedAction(Runnable command) {
			return true;
		}
	 ...
	 */
	private static final class AbortWhenBlocked
			implements BlockedExecutionHandler {
		public boolean blockedAction(Runnable command) {
	 ...
	 */
	private final class DiscardOldestWhenBlocked
			implements BlockedExecutionHandler {
		public boolean blockedAction(Runnable command)
				throws InterruptedException {
			_handOff.poll(0);
			if(!_handOff.offer(command, 0)) {
				command.run();
			}
			return true;
		}
	...
	public void execute(Runnable command) throws InterruptedException {
		for(;;) {
			synchronized(this) {
				if(!_shutdown) {
					final int size = _poolSize;
					// Ensure minimum number of threads
					if(size < _minimumPoolSize) {
						addThread(command);
						return;
					}
