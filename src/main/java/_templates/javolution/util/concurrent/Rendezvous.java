...
public class Rendezvous implements Barrier {
	...
	public static class Rotator implements RendezvousFunction {
		 ...
		 */
		public void rendezvousFunction(Object[] objects) {
			final int lastIdx = objects.length - 1;
			final Object first = objects[0];
	 ...
	 */
	private long _departures = 0;
	/**
	 * Incoming threads pile up on entry until last set done.
	 */
	private final Semaphore _entryGate;
	/**
	 ...
	 */
	private RendezvousFunction _rendezvousFunction;
	/**
	...
	public Rendezvous(int parties, RendezvousFunction function) {
		...
		_parties = parties;
		_rendezvousFunction = function;
		_entryGate = new WaiterPreferenceSemaphore(parties);
		_slots = new Object[parties];
	}
	/**
	...
	public synchronized RendezvousFunction setRendezvousFunction(
		...
		final RendezvousFunction old = _rendezvousFunction;
		_rendezvousFunction = function;
		return old;
	}
	public int parties() {
	...
	private Object doRendezvous(Object x, boolean timed, long msecs)
			throws InterruptedException, TimeoutException,
			BrokenBarrierException {
		...
		synchronized(this) {
			Object y = null;
			final int index = _entries++;
			_slots[index] = x;
			try {
				// last one in runs function and releases
				if(_entries == _parties) {
					_departures = _entries;
					notifyAll();
					try {
						if(!_broken && _rendezvousFunction != null) {
							_rendezvousFunction.rendezvousFunction(_slots);
						}
					}
					...
					while(!_broken && _departures < 1) {
						...
						try {
							...
							if(_broken || _departures > 0) { // interrupted after release
								Thread.currentThread().interrupt();
								break;
							}
							_broken = true;
