...
public class FJTaskRunnerGroup implements Executor {
	 ...
	 */
	private final FJTaskRunner[] threads;
	/**
	 * Group-wide queue for tasks entered via execute()
	 */
	private final LinkedQueue entryQueue = new LinkedQueue();
	/**
	 ...
	 * @throws InterruptedException if current Thread is
	...
	public void invoke(Runnable r) throws InterruptedException {
		final InvokableFJTask w = new InvokableFJTask(r);
		entryQueue.put(w);
		signalNewTask();
		w.awaitTermination();
	}
	...
	public synchronized void setScanPriorities(int pri) {
		for(int i = 0; i < threads.length; ++i) {
			final FJTaskRunner t = threads[i];
	...
	public void stats() {
		...
		for(int i = 0; i < threads.length; ++i) {
			final FJTaskRunner t = threads[i];
			final int truns = t.runs;
			totalRuns += truns;
			final int tscans = t.scans;
			totalScans += tscans;
			final int tsteals = t.steals;
			totalSteals += tsteals;
			final String star = getActive(t) ? "*" : " ";
			System.out.print("T" + i + star + "\t" + t.deqSize() + "\t" + tscans
					+ "\t" + tsteals + "\t" + truns + "\n");
		}
		...
		long rps = 0;
		if(secs != 0) {
			rps = MathLib.round(totalRuns / secs);
		}
	 ...
	 * Called only by FJTaskRunner.scan().
	 */
	final FJTask pollEntryQueue() {
		try {
			final FJTask t = (FJTask) entryQueue.poll(0);
			return t;
		}
		catch(final InterruptedException ex) { // ignore interrupts
			Thread.currentThread().interrupt();
			return null;
		}
	}
	 ...
	 */
	static final synchronized boolean getActive(FJTaskRunner t) {
		return t.active;
	}
	/**
	 ...
	 */
	final synchronized void setActive(FJTaskRunner t) {
		if(!t.active) {
			t.active = true;
			++activeCount;
	 ...
	 */
	final synchronized void setInactive(FJTaskRunner t) {
		if(t.active) {
			t.active = false;
			--activeCount;
		}
	 ...
	 */
	final synchronized void checkActive(FJTaskRunner t, long scans) {
		setInactive(t);
		try {
			...
			if(activeCount == 0 && entryQueue.peek() == null) {
				...
				final int nsecs = msecs == 0 ? 1 : 0; // forces shortest possible sleep
				wait(msecs, nsecs);
			}
		}
		catch(final InterruptedException ex) {
	 ...
	 */
	final synchronized void signalNewTask() {
		if(COLLECT_STATS) {
			++entries;
		}
		if(nstarted < threads.length) {
	 ...
	 */
	private static final class InvokableFJTask extends FJTask {
		private final Runnable wrapped;
		private boolean terminated = false;
		private InvokableFJTask(Runnable r) {
			wrapped = r;
		}
		public void run() {
			try {
