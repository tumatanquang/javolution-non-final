 ...
 * implement    read-after-writeof 2 volatiles.
 */
 ...
 * in <A href="//supertech.lcs.mit.edu/cilk/"> Cilk</A> and
 * <A href="//www.cs.utexas.edu/users/hood/"> Hood</A>, and
 * to a lesser extent
 * <A href="//www.cs.uga.edu/~dkl/filaments/dist.html"> Filaments</A>,
...
public class FJTaskRunner extends Thread {
	 ...
	 */
	private final FJTaskRunnerGroup group;
	/**
	 ...
	 */
	private static final class VolatileTaskRef {
		/**
		 * The reference
		 */
		volatile FJTask ref;
		 ...
		 * VolatileTaskRefs.
		 */
		static VolatileTaskRef[] newArray(int cap) {
			final VolatileTaskRef[] a = new VolatileTaskRef[cap];
			for(int k = 0; k < cap; k++) {
	 ...
	 */
	private int scanPriority = FJTaskRunnerGroup.DEFAULT_SCAN_PRIORITY;
	/**
	 ...
	 */
	final synchronized void slowPush(final FJTask r) {
		checkOverflow();
	...
	final synchronized void put(final FJTask r) {
		for(;;) {
			final int b = base - 1;
			if(top < b + deq.length) {
				final int newBase = b & deq.length - 1;
				deq[newBase].put(r);
				base = newBase;
	 ...
	 */
	final FJTask confirmTake(int oldBase) {
		/*
	...
	final void checkOverflow() {
		...
		if(t - b < deq.length - 1) { // check if just need an index reset
			...
			final int newTop = t - b;
			final int oldcap = deq.length;
			final int newcap = oldcap * 2;
			if(newcap >= MAX_CAPACITY)
				throw new Error("FJTask queue maximum capacity exceeded");
			final VolatileTaskRef[] newdeq = new VolatileTaskRef[newcap];
	 ...
	 */
	final void scan(final FJTask waitingFor) {
		FJTask task = null;
		 ...
		 */
		final FJTaskRunner[] ts = group.getArray();
		int idx = victimRNG.nextInt(ts.length);
		for(int i = 0; i < ts.length; ++i) {
			final FJTaskRunner t = ts[idx];
			if(++idx >= ts.length) {
	 ...
	 */
	final void slowCoInvoke(final FJTask w, final FJTask v) {
	...
	final void coInvoke(FJTask[] tasks) {
		final int nforks = tasks.length - 1;
	 ...
	 */
	final void slowCoInvoke(FJTask[] tasks) {
