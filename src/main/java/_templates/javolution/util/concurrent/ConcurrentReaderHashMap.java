...
/*@JVM-1.1+@import _templates.java.lang.CloneNotSupportedException;/**/
...
public class ConcurrentReaderHashMap extends AbstractMap
	 ...
	 */
	private static final class BarrierLock implements Serializable {}
	/**
	 * Lock used only for its memory effects.
	 */
	private final BarrierLock barrierLock = new BarrierLock();
	/**
	 * field written to only to guarantee lock ordering.
	 */
	private transient Object lastWrite;
	/**
	 * Force a memory synchronization that will cause
	 * all readers to see table. Call only when already
	 * holding main synch lock.
	 */
	private final void recordModification(Object x) {
	 ...
	 * use of barrierLock
	 */
	private final Entry[] getTableForReading() {
		synchronized(barrierLock) {
			return table;
		}
	}
	 ...
	 */
	private static final int p2capacity(int initialCapacity) {
	...
	public Object get(Object key) {
		...
		for(;;) {
			if(e == null) {
				...
				final Entry[] reread = getTableForReading();
			...
			if(tab == table) {
				if(e == null) {
					...
					if(first == tab[index]) {
						...
						final Entry newEntry = new Entry(hash, key, value,
								first);
						tab[index] = newEntry;
						if(++count >= threshold) {
							rehash();
	 ...
	 */
	final Object sput(Object key, Object value, int hash) {
		final Entry[] tab = table;
		final int index = hash & tab.length - 1;
		final Entry first = tab[index];
		Entry e = first;
	 ...
	 * held and interference has been detected.
	 */
	final Object sremove(Object key, int hash) {
		final Entry[] tab = table;
		final int index = hash & tab.length - 1;
		final Entry first = tab[index];
	...
	public Object putIfAbsent(Object key, Object value) {
		...
		synchronized(this) {
			...
			// Wrong list or interference; retry under lock
			return sputIfAbsent(key, value, hash);
		}
	}
	final Object sputIfAbsent(Object key, Object value, int hash) {
		final Entry[] tab = table;
		final int index = hash & tab.length - 1;
		final Entry first = tab[index];
		for(Entry e = first; e != null; e = e.next) {
	...
	public boolean replace(Object key, Object oldValue, Object newValue) {
		...
		synchronized(this) {
			...
			// Wrong list or interference; retry under lock
			return sreplace(key, oldValue, newValue, hash);
		}
	}
	final boolean sreplace(Object key, Object oldValue, Object newValue,
			int hash) {
		final Entry[] tab = table;
		final int index = hash & tab.length - 1;
	...
	public Object replace(Object key, Object value) {
		...
		synchronized(this) {
			...
			// Wrong list or interference; retry under lock
			return sreplace(key, value, hash);
		}
	}
	final Object sreplace(Object key, Object value, int hash) {
		final Entry[] tab = table;
		final int index = hash & tab.length - 1;
		for(Entry e = tab[index]; e != null; e = e.next) {
			if(e.hash == hash && eq(key, e.key)) {
	...
	public boolean containsValue(Object value) {
		if(value == null)
			throw new NullPointerException();
		final Entry tab[] = getTableForReading();
	...
	/**
	 * Returns a {@link KeySetView} backing a new {@link ConcurrentReaderHashMap}.
	 */
	public static KeySetView newKeySet() {
		return new KeySetView(new ConcurrentReaderHashMap(), Boolean.TRUE);
	}
	/**
	 * Returns a {@link KeySetView} backing a new {@link ConcurrentReaderHashMap} of the given capacity.
	 */
	public static KeySetView newKeySet(int initialCapacity) {
		return new KeySetView(new ConcurrentReaderHashMap(initialCapacity),
				Boolean.TRUE);
	}
	/**
	 ...
	 */
	final synchronized boolean findAndRemoveEntry(Map.Entry entry) {
		final Object key = entry.getKey();
	...
	class HashIterator implements Iterator, Enumeration {
		final Entry[] tab; // snapshot of table
		int index; // current slot
		Entry entry = null; // current node of slot
		Object currentKey; // key for current node
		Object currentValue; // value for current node
		Entry lastReturned = null; // last node returned by next
		HashIterator() {
	...
	/**
	 * A Set view of the keys of a ConcurrentReaderHashMap, where additions are enabled
	 * by mapping to a common value.
	 */
	public static final class KeySetView extends AbstractSet
			implements Set, Serializable {
		private final ConcurrentReaderHashMap _map;
		private final Object _value;
		KeySetView(ConcurrentReaderHashMap map, Object value) {
			_map = map;
			_value = value;
		}
		...
		public ConcurrentReaderHashMap getMap() {
			return _map;
		}
		public void clear() {
			_map.clear();
