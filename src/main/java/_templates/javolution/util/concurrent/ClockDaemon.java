...
public class ClockDaemon extends ThreadFactoryUser {
	...
	private final Heap _heap = new Heap(DefaultChannelCapacity.get());
	private static final class TaskNode implements Comparable {
		final Runnable command; // The command to run
		final long period; // The cycle period, or -1 if not periodic
		private long _timeToRun; // The time to run command
		...
		public int compareTo(Object other) {
			final long a = getTimeToRun();
			final long b = ((TaskNode) other).getTimeToRun();
	...
	public Object executeAt(Date date, Runnable command) {
		final TaskNode task = new TaskNode(date.getTime(), command);
		_heap.insert(task);
		restart();
		return task;
	}
	...
	public Object executeAfterDelay(long millisecondsToDelay,
			Runnable command) {
		final long runtime = System.currentTimeMillis() + millisecondsToDelay;
		final TaskNode task = new TaskNode(runtime, command);
		_heap.insert(task);
		restart();
		return task;
	}
	...
	public Object executePeriodically(long period, Runnable command,
		...
		final TaskNode task = new TaskNode(firstTime, command, period);
		_heap.insert(task);
		restart();
		return task;
	}
	 ...
	 * @throws ClassCastException if the taskID argument is not
	 ...
	 */
	private synchronized TaskNode nextTask() {
		...
		if(true) {
			try {
				while(!Thread.interrupted()) {
					...
					TaskNode task = (TaskNode) _heap.peek();
					if(task == null) {
						wait();
					}
					else {
						final long now = System.currentTimeMillis();
						final long when = task.getTimeToRun();
						if(when > now) { // false alarm wakeup
							wait(when - now);
						}
						else {
							task = (TaskNode) _heap.extract();
