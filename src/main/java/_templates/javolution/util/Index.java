...
public final class Index extends Number implements Comparable/*<Index>*/,
		Record, Realtime, Immutable, XMLSerializable {
	/**
	 ...
	 */
	public static final Configurable/*<Integer>*/ INITIAL_FIRST = new Configurable(
			new Integer(-(_NegativeIndicesLength - 1))) {
	...
	static {
		...
		final int len = _PositiveIndices.length;
		for(int i = 1; i < len; ++i) {
	 ...
	 */
	public static final Configurable/*<Integer>*/ INITIAL_LAST = new Configurable(
			new Integer(_PositiveIndicesLength - 1)) {
		protected void notifyChange(Object oldValue, Object newValue) {
	 ...
	 * immortal memory).
	 */
	private static final MemoryArea IMMORTAL_MEMORY = MemoryArea
			.getMemoryArea(new Object());
	/**
	...
	public static Index valueOf(int i) { // Short to be inlined.
		return i >= 0
				? i < _PositiveIndicesLength ? _PositiveIndices[i]
						: createPositive(i)
				: valueOfNegative(-i);
	}
	/**
	...
	public static List/*<Index>*/ rangeOf(int start, int end) {
		final FastTable/*<Index>*/ list = new FastTable(end - start);
	...
	public static List<Index> valuesOf(int... indices) {
		...
		FastTable<Index> list = new FastTable(len);
		for(int i = 0; i < len; ++i) {
			list.add(Index.valueOf(indices[i]));
		}
		return  list;
	}
	/**/
	private static final Index valueOfNegative(int i) {
		return i < _NegativeIndicesLength ? _NegativeIndices[i]
				: createNegative(i);
	}
	private static final synchronized Index createPositive(int i) {
	...
	private static final synchronized Index createNegative(int i) {
	...
	private static final Runnable AUGMENT_POSITIVE = new Runnable() {
		public final void run() {
			final int n = _PositiveIndicesLength + INCREASE_AMOUNT;
			for(int i = _PositiveIndicesLength; i < n; ++i) {
				final Index index = new Index(i);
				if(_PositiveIndices.length <= i) { // Resize.
					final Index[] tmp = new Index[_PositiveIndices.length * 2];
					System.arraycopy(_PositiveIndices, 0, tmp, 0,
							_PositiveIndices.length);
	...
	private static final Runnable AUGMENT_NEGATIVE = new Runnable() {
		public final void run() {
			final int n = _NegativeIndicesLength + INCREASE_AMOUNT;
			for(int i = _NegativeIndicesLength; i < n; ++i) {
				final Index index = new Index(-i);
				if(_NegativeIndices.length <= i) { // Resize.
					final Index[] tmp = new Index[_NegativeIndices.length << 1];
					System.arraycopy(_NegativeIndices, 0, tmp, 0,
							_NegativeIndices.length);
	...
	public final int compareTo(Object/*{Index}*/ that) {
		return _value - ((Index) that)._value;
	}
