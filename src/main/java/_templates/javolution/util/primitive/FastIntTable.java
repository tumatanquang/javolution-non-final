...
/**
 * <p> This class represents a high-performance, resizable table of <code>int</code>
 ...
 *     {@link FastIntTable} increases its capacity smoothly by allocating fixed-size
 ...
 */
public class FastIntTable extends FastPrimitiveCollection
	 ...
	 */
	private transient int[] _low;
	/**
	 * For larger capacity use multi-dimensional array of 1024-element blocks.
	 */
	private transient int[][] _high;
	/**
	 ...
	 */
	public FastIntTable(int capacity) {
		_capacity = C0;
		_low = new int[C0];
		_high = new int[1][];
		_high[0] = _low;
	 ...
	 */
	public FastIntTable(int[] values) {
	...
	private final void increaseCapacity() {
		if(_capacity < C1) {
			_capacity <<= 1;
			final int[] tmp = new int[_capacity];
			...
			if(j >= _high.length) {
				final int[][] tmp = new int[_high.length << 1][];
	...
	public int set(int index, int value) {
		if(index < 0 || index >= size)
			throw new ArrayIndexOutOfBoundsException(
					"Index: " + index + ", Size: " + size);
		final int[] low = _high[index >> B1];
		final int previous = low[index & M1];
		low[index & M1] = value;
		return previous;
	}
	/**
	 ...
	 * @return <code>true</code> if this table changed as a result of the call.
	 */
	public boolean addAll(FastIntTable values) {
		return addAll(size, values);
	}
	/**
	 ...
	 * @throws ArrayIndexOutOfBoundsException if index is out of range.
	 */
	public boolean addAll(int index, FastIntTable values) {
		if(index < 0 || index > size)
			throw new ArrayIndexOutOfBoundsException(
					"Index: " + index + ", Size: " + size);
	 ...
	 * @return the index of the first occurrence of the element, or -1.
	 */
	public int indexOf(int value) {
		for(int i = 0; i < size;) {
			final int[] low = _high[i >> B1];
	 ...
	 *
	 * @param value element to search for.
	 * @return the index of the last occurrence of the element, or -1.
	 */
	public int lastIndexOf(int value) {
		for(int i = size; i > 0;) {
			final int block = i - 1 >> B1;
			final int[] low = _high[block];
	...
	private final void quicksort(int first, int last) {
		int i = first;
		int j = last;
		final int piv = get(first + last >> 1);
		do {
			while(get(i) < piv) {
				++i;
			}
	...
	public FastPrimitiveCollection/*FastIntTable*/ unmodifiable() {
		return new Unmodifiable(this);
	}
	public FastPrimitiveCollection/*FastIntTable*/ shared() {
		return new Shared(this);
	}
	public FastPrimitiveIterator/*FastIntIterator*/ iterator() {
		return new IntIterator(this);
	}
	public Object/*FastIntTable*/ clone() throws CloneNotSupportedException {
		/*@JVM-1.1+@
		if(true) {
			final FastIntTable c = (FastIntTable) super.clone();
			c._capacity = C0;
			c._low = new int[C0];
			c._high = new int[1][];
			c._high[0] = c._low;
	...
	public boolean equals(Object o) {
		if(this == o)
			return true;
		if(!(o instanceof FastIntTable))
			return false;
		final FastIntTable that = (FastIntTable) o;
	...
	private static final class IntIterator implements FastIntIterator {
		private final FastIntTable _table;
		private int _index;
		private int _last = -1;
		private IntIterator(FastIntTable table) {
			_table = table;
		}
	...
	private static final class Unmodifiable extends FastIntTable
			implements Cloneable, RandomAccess, Reusable, Serializable {
		private final FastIntTable _table;
		private Unmodifiable(FastIntTable table) {
			super(0);
			_table = table;
		}
		public final FastPrimitiveCollection/*FastIntTable*/ unmodifiable() {
			return this;
		}
		public final FastPrimitiveCollection/*FastIntTable*/ shared() {
		...
		public final Object/*FastIntTable*/ clone()
		...
		public final boolean addAll(FastIntTable values) {
			throw new UnsupportedOperationException("Unmodifiable");
		}
		public final boolean addAll(int index, FastIntTable values) {
			throw new UnsupportedOperationException("Unmodifiable");
		}
		...
		public final void addLast(int value) {
			throw new UnsupportedOperationException("Unmodifiable");
		}
		...
		public final boolean offer(int value) {
			throw new UnsupportedOperationException("Unmodifiable");
		}
		...
		public final FastPrimitiveIterator/*FastIntIterator*/ iterator() {
			return new FastIntIterator() {
				private final FastIntIterator _it = (FastIntIterator) _table
						.iterator();
				public final boolean hasNext() {
					return _it.hasNext();
				}
	...
	private static final class Shared extends FastIntTable
			implements Cloneable, RandomAccess, Reusable, Serializable {
		private final FastIntTable _table;
		private final ReadWriteLock _lock;
		private Shared(FastIntTable table) {
			super(0);
			_table = table;
		...
		public final FastPrimitiveIterator/*FastIntIterator*/ iterator() {
			final FastIntIterator iterator = (FastIntIterator) _table
					.iterator();
			return new FastIntIterator() {
				public final boolean hasNext() {
					boolean wasInterrupted = /*@JVM-1.1+@ true ? Thread.interrupted() : /**/false;
