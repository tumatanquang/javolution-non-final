 ...
 *     {@link String} and {@link _templates.javolution.text.Text Text}
 ...
 * FastMap identityMap = new FastMap().setKeyComparator(FastComparator.IDENTITY);[/code]</p>
 *
...
public class FastMap
		/*<K,V>*/ implements Map/*<K,V>*/, Reusable, XMLSerializable, Realtime {
	...
	private final void setup(int capacity) {
		int tableLength = C0;
	...
	public final FastMap/*<V,K>*/ reverse() {
		final FastMap/*<V,K>*/ map = (FastMap/*<V,K>*/) FACTORY.object();
	...
	public final Object/*{V}*/ put(Object/*{K}*/ key, Object/*{V}*/ value) {
		return (Object/*{V}*/) put(key, value,
				_isDirectKeyComparator ? key.hashCode()
						: _keyComparator.hashCodeOf(key),
				_isShared, false, false);
	}
	...
	private final void createNewEntries() { // Increase the number of entries.
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
				Entry previous = _tail;
				for(int i = 0; i < 8; ++i) { // Creates 8 entries at a time.
					final Entry/*<K,V>*/ newEntry = newEntry();
					newEntry._previous = previous;
					previous._next = newEntry;
					previous = newEntry;
				}
	...
	private final void resizeTable(final boolean isShared) {
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
				...
				if(nullCount > _entryCount) { // Yes.
					if(isShared) { // Replaces with a new table.
						final Entry[] tmp = new Entry[_entries.length];
						copyEntries(_entries, tmp, _entries.length);
						_entries = tmp;
					}
					else { // We need a temporary buffer.
						final Object[] tmp = (Object[]) ArrayFactory.OBJECTS_FACTORY
				...
				if(tableLength <= C1) { // Ok to resize.
					final Entry[] tmp = new Entry[tableLength];
					copyEntries(_entries, tmp, _entries.length);
					_entries = tmp;
					return;
				}
				...
				for(int i = 0; i < _entries.length;) {
					final Entry entry = _entries[i++];
					if(entry == null || entry == Entry.NULL) {
						continue;
					}
					final FastMap subMap = _subMaps[entry._keyHash >> _keyShift
							& C2 - 1];
					subMap.mapEntry(entry);
					if(subMap._entryCount
							+ subMap._nullCount << 1 >= subMap._entries.length) {
	...
	private final FastMap[] newSubMaps(int capacity) {
		final FastMap[] subMaps = new FastMap[C2];
		for(int i = 0; i < C2; ++i) {
			final FastMap subMap = new FastMap(new Entry[capacity]);
	...
	private final void mapEntry(Entry entry) {
		final int mask = _entries.length - 1;
		for(int i = entry._keyHash >> _keyShift;; ++i) {
			final Entry e = _entries[i & mask];
			if(e == null) {
	...
	private final void copyEntries(Object[] from, Entry[] to, int count) {
		final int mask = to.length - 1;
		for(int i = 0; i < count;) {
			final Entry entry = (Entry) from[i++];
			if(entry == null || entry == Entry.NULL) {
				continue;
			}
			for(int j = entry._keyHash >> _keyShift;; ++j) {
				final Entry tmp = to[j & mask];
				if(tmp == null) {
					to[j & mask] = entry;
					break;
				}
	 ...
	 * @throws NullPointerException if the key is <code>null</code>.
	 */
	public final Entry/*<K,V>*/ putEntry(Object/*{K}*/ key,
			Object/*{V}*/ value) {
		return (Entry/*<K,V>*/) put(key, value,
				_isDirectKeyComparator ? key.hashCode()
						: _keyComparator.hashCodeOf(key),
	...
	public final void putAll(Map/*<? extends K, ? extends V>*/ map) {
		for(final Iterator i = map.entrySet().iterator(); i.hasNext();) {
			final Map.Entry/*<K,V>*/ e = (Map.Entry/*<K,V>*/) i.next();
	...
	public final Object/*{V}*/ putIfAbsent(Object/*{K}*/ key,
		...
		return (Object/*{V}*/) put(key, value,
				_isDirectKeyComparator ? key.hashCode()
						: _keyComparator.hashCodeOf(key),
				_isShared, true, false);
	}
	/**
	 * Removes the entry for the specified key if present. The entry
	 * is recycled if the map is not marked as {@link #shared shared()};
	...
	public FastMap/*<K,V>*/ setKeyComparator(
		...
		_isDirectKeyComparator = keyComparator == FastComparator.DIRECT
				|| _keyComparator == FastComparator.DEFAULT
						&& !((Boolean) FastComparator.REHASH_SYSTEM_HASHCODE
								.get()).booleanValue();
		return this;
	}
	/**
	 ...
	 * unless this map is {@link #shared shared()} in which case the entries
	...
	private final synchronized void clearShared() {
	...
	public void printStatistics(PrintStream out) {
		final long sum = getSumDistance();
		final int size = size();
		final int avgDistancePercent = size != 0 ? (int) (100 * sum / size) : 0;
		synchronized(out) {
			out.print("SIZE: " + size);
	...
	private final int getTableLength() {
		if(_useSubMaps) {
			int sum = 0;
			for(int i = 0; i < C2; ++i) {
	...
	private final int getCapacity() {
		int capacity = 0;
		for(Entry e = _head; (e = e._next) != null; ++capacity) {}
		return capacity - 1;
	}
	private final int getMaximumDistance() {
		int max = 0;
		if(_useSubMaps) {
			for(int i = 0; i < C2; ++i) {
				final int subMapMax = _subMaps[i].getMaximumDistance();
				max = MathLib.max(max, subMapMax);
			}
			return max;
		}
		final int len = _entries.length;
		for(int i = 0; i < len; ++i) {
			final Entry entry = _entries[i];
			if(entry != null && entry != Entry.NULL) {
				final int slot = entry._keyHash >> _keyShift & len - 1;
	...
	private final long getSumDistance() {
		long sum = 0;
		if(_useSubMaps) {
			for(int i = 0; i < C2; ++i) {
	...
	private final int getSubMapDepth() {
		if(_useSubMaps) {
			int depth = 0;
			for(int i = 0; i < C2; ++i) {
				final int subMapDepth = _subMaps[i].getSubMapDepth();
	...
	private static final void reset(Object[] entries) {
		final int len = entries.length;
		for(int i = 0; i < len;) {
			final int count = MathLib.min(len - i, C1);
	...
	*/
	...
	private static final class ValueIterator implements Iterator {
		private static final ObjectFactory FACTORY = new ObjectFactory() {
			...
			protected void cleanup(Object obj) {
				final ValueIterator iterator = (ValueIterator) obj;
				iterator._map = null;
		...
		public static ValueIterator valueOf(FastMap map) {
			final ValueIterator iterator = (ValueIterator) ValueIterator.FACTORY
					.object();
			iterator._map = map;
	...
	private final class EntrySet extends FastCollection implements Set {
		...
		public boolean contains(Object obj) { // Optimization.
			if(obj instanceof Map.Entry) {
				...
				final Entry thisEntry = getEntry(thatEntry.getKey());
				if(thisEntry == null)
					return false;
			...
			public boolean areEqual(Object o1, Object o2) {
				if(o1 instanceof Map.Entry && o2 instanceof Map.Entry) {
					...
					return _keyComparator.areEqual(e1.getKey(), e2.getKey())
							&& _valueComparator.areEqual(e1.getValue(),
									e2.getValue());
				}
	...
	private static final class EntryIterator implements Iterator {
		private static final ObjectFactory FACTORY = new ObjectFactory() {
			...
			protected void cleanup(Object obj) {
				final EntryIterator iterator = (EntryIterator) obj;
				iterator._map = null;
				iterator._current = null;
				iterator._next = null;
				iterator._tail = null;
		...
		public static EntryIterator valueOf(FastMap map) {
			final EntryIterator iterator = (EntryIterator) EntryIterator.FACTORY
					.object();
			iterator._map = map;
			iterator._next = map._head._next;
			iterator._tail = map._tail;
	...
	private static final class KeyIterator implements Iterator {
		private static final ObjectFactory FACTORY = new ObjectFactory() {
			...
			protected void cleanup(Object obj) {
				final KeyIterator iterator = (KeyIterator) obj;
				iterator._map = null;
				iterator._current = null;
				iterator._next = null;
				iterator._tail = null;
		...
		public static KeyIterator valueOf(FastMap map) {
			final KeyIterator iterator = (KeyIterator) KeyIterator.FACTORY
					.object();
			iterator._map = map;
			iterator._next = map._head._next;
			iterator._tail = map._tail;
	...
	public static class Entry
			/*<K,V>*/ implements Map.Entry/*<K,V>*/, Record, Realtime {
		/**
		...
		public boolean equals(Object that) {
			if(that instanceof Map.Entry) {
				...
				return FastComparator.DEFAULT.areEqual(_key, entry.getKey())
						&& FastComparator.DEFAULT.areEqual(_value,
								entry.getValue());
			}
			return false;
		}
