...
public abstract class FastComparator
		/*<T>*/ implements Comparator/*<T>*/, XMLSerializable {
	/**
	...
	private static boolean _Rehash = ((Boolean) REHASH_SYSTEM_HASHCODE.get())
			.booleanValue();
	private static final boolean isPoorSystemHash() {
		final boolean[] dist = new boolean[64]; // Length power of 2.
		final int len = dist.length;
		for(int i = 0; i < len; ++i) {
			dist[new Object().hashCode() & len - 1] = true;
		}
		int occupied = 0;
		for(int i = 0; i < len;) {
			occupied += dist[i++] ? 1 : 0; // Count occupied slots.
		}
		return occupied < len >> 2; // Less than 16 slots on 64.
	}
	/**
	...
	private static final class Default/*<T>*/ extends FastComparator/*<T>*/ {
		public int hashCodeOf(Object/*{T}*/ obj) {
			return obj == null ? 0
					: _Rehash ? REHASH.hashCodeOf(obj) : obj.hashCode();
		}
