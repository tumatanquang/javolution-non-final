...
import _templates.java.io.CharConversionException;
...
public final class UTF8StreamReader extends Reader implements Reusable {
	 ...
	 * For example:[code]
	 * Reader reader = new UTF8StreamReader().setInput(inStream);[/code]
	 * is equivalent but reads twice as fast as [code]
	 * Reader reader = new java.io.InputStreamReader(inStream, "UTF-8");[/code]
	 *
			...
			if(b >= 0) {
				...
				if(code < 0x10000) {
					...
					throw new CharConversionException(
							"Cannot convert U+" + Integer.toHexString(code)
									+ " to char (code greater than U+10FFFF)");
			}
		}
	}
