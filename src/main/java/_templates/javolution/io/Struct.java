 ...
 *     address using
 *     <a href="//java.sun.com/docs/books/tutorial/native1.1/index.html">
 ...
 * }[/code]
 *     Below is the <code>nativeBuffer()</code> implementation (<code>Clock.c</code>):[code]
 * #include <jni.h>
 ...
 *     from the leftmost to the rightmost bit (same as <code>BIG_ENDIAN</code>).</p>
 *
...
public class Struct {
	 ...
	 */
	public static final Configurable/*<Integer>*/ MAXIMUM_ALIGNMENT = new Configurable(
			new Integer(4)) {};
	/**
	...
	public int read(InputStream in) throws IOException {
		...
		if(buffer.hasArray()) {
			final int offset = buffer.arrayOffset() + getByteBufferPosition();
		...
		synchronized(buffer) {
			...
			final int bytesRead = in.read(_bytes);
	...
	public final long address() {
		final ByteBuffer thisBuffer = getByteBuffer();
		if(ADDRESS_METHOD != null) {
			final Long start = (Long) ADDRESS_METHOD.invoke(thisBuffer);
	...
	public String toString() {
		...
		try {
			...
			for(int i = 0; i < size; ++i) {
				final int b = buffer.get(start + i) & 0xFF;
				tmp.append(HEXA[b >> 4]);
	...
	protected/* <S extends Struct> S*/ Struct[] array(/*S*/Struct[] structs) {
		...
		final boolean resetIndexSaved = _resetIndex;
		if(_resetIndex) {
			_index = 0;
			_resetIndex = false; // Ensures the array elements are sequential.
		}
		final int len = structs.length;
		for(int i = 0; i < len;) {
			...
			if(struct == null) {
				try {
					if(structClass == null) {
						...
						final String structName = arrayName.substring(2,
								arrayName.length() - 1);
						structClass = Reflection.getInstance()
								.getClass(structName);
						if(structClass == null)
							throw new IllegalArgumentException("Struct class: "
	...
	protected/* <M extends Member> M*/ Member[] array(
		...
		if(BOOL.isInstance(arrayMember)) {
			final int len = arrayMember.length;
			for(int i = 0; i < len;) {
	...
	public void writeBits(long value, int bitOffset, int bitSize) {
		...
		final int offset = bitOffset >> 3;
		final int bitStart = byteOrder() == ByteOrder.BIG_ENDIAN
				? bitOffset - (offset << 3)
		...
		final long newValue = resetValue | value << 64 - bitSize - bitStart;
	...
	private static void writeByte(int index, ByteBuffer byteBuffer,
			byte value) {
			...
			synchronized(buffer) {
				...
				try {
					...
					for(int i = 0; i < _length; ++i) {
						final char c = (char) _reader.read();
						if(c == 0)
							return tmp.toString();
						tmp.append(c);
					}
	...
	public class Bool extends Member {
		...
		public void set(boolean value) {
			...
			case 8:
				...
				break;
			default:
				getByteBuffer().put(index, (byte) set(value ? -1 : 0, 1,
						getByteBuffer().get(index)));
				break;
			}
		}
		public String toString() {
	...
	public class Signed8 extends Member {
		...
		public byte get() {
			final int index = getByteBufferPosition() + offset();
			final int word = getByteBuffer().get(index);
	...
	public class Signed16 extends Member {
		...
		public short get() {
			final int index = getByteBufferPosition() + offset();
			final int word = getByteBuffer().getShort(index);
	...
	public class Signed32 extends Member {
		...
		public int get() {
			final int index = getByteBufferPosition() + offset();
			final int word = getByteBuffer().getInt(index);
	...
	public class Signed64 extends Member {
		...
		public long get() {
			final int index = getByteBufferPosition() + offset();
			final long word = getByteBuffer().getLong(index);
	...
	public class BitField extends Member {
		...
		public long longValue() {
			final long signedValue = readBits(bitIndex() + (offset() << 3),
					bitLength());
	...
	public class Reference64/*<S extends Struct>*/ extends Member {
		...
		public boolean isUpToDate() {
			...
			return getByteBuffer().getLong(index) == 0;
		}
	}
	/**
	...
	public class Enum8/*<T extends Enum>*/ extends Member {
		...
		public void set(Enum/*T*/ e) {
			...
			if(_values[value] != e)
				throw new IllegalArgumentException("enum: " + e
