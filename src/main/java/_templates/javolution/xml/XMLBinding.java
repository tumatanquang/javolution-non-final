...
public class XMLBinding implements Reusable, XMLSerializable {
	...
	protected Class readClass(XMLStreamReader reader, boolean useAttributes)
		...
		if(useAttributes) {
			...
			classQName = QName.valueOf(
					reader.getAttributeValue(_classAttribute.getNamespaceURI(),
		...
		cls = (Class) _aliasToClass
				.get(QName.valueOf(classQName.getLocalName()));
		if(cls != null)
			return cls;
	...
	protected void writeClass(Class cls, XMLStreamWriter writer,
		...
		final QName qName = (QName) _classToAlias.get(cls);
		final String name = qName != null ? qName.toString() : cls.getName();
		if(useAttributes) {
			if(_classAttribute == null)
				return;
	...
	static final XMLFormat OBJECT_XML = new XMLFormat(Object.class) {
		...
		public Object newInstance(Class cls,
				...
				throws XMLStreamException {
			final TextFormat format = TextFormat.getInstance(cls);
			if(!format.isParsingSupported())
				throw new XMLStreamException(
						"No XMLFormat or TextFormat (with parsing supported) for instances of "
								+ cls);
			final CharArray value = xml.getAttribute("value");
			if(value == null)
				throw new XMLStreamException(
	 ...
	 */
	static final XMLFormat MAP_XML = new XMLFormat(
			_templates.java.util.Map.class) {
		public void read(InputElement xml, Object obj)
				throws XMLStreamException {
			final Map map = (Map) obj;
			while(xml.hasNext()) {
				final Object key = xml.get("Key");
