...
import _templates.javolution.xml.stream.XMLStreamConstants;
...
public class XMLReaderImpl implements XMLReader, Reusable {
	 ...
	 *         supplied by the application.
	 */
	public void parse(InputStream in, String encoding)
	...
	public void parse(InputSource input) throws IOException, SAXException {
		...
		if(reader != null) {
			...
			final InputStream inStream = input.getByteStream();
			if(inStream != null) {
	...
	public void parse(String systemId) throws IOException, SAXException {
		...
		try {
			final Object url = NEW_URL.newInstance(systemId);
			inStream = (InputStream) OPEN_STREAM.invoke(url);
		}
		catch(final Exception urlException) { // Try as filename.
			try {
				inStream = (InputStream) NEW_FILE_INPUT_STREAM
						.newInstance(systemId);
			}
			catch(final Exception fileException) {
				throw new UnsupportedOperationException(
						"Cannot parse " + systemId);
			}
		}
		parse(inStream);
	}
	...
	public void setFeature(String name, boolean value)
			throws SAXNotRecognizedException, SAXNotSupportedException {
		if(name.equals("http://xml.org/sax/features/namespaces") || name
	...
	private void parseAll() throws XMLStreamException, SAXException {
		final int eventType = _xmlReader.getEventType();
		...
		while(doContinue) {
			...
			case XMLStreamConstants.START_ELEMENT:
				...
				int count = _xmlReader.getNamespaceCount();
			...
			case XMLStreamConstants.END_ELEMENT:
				...
				// End prefix mapping.
				count = _xmlReader.getNamespaceCount();
				for(int i = 0; i < count; ++i) {
					prefix = _xmlReader.getNamespacePrefix(i);
					prefix = prefix == null ? NO_CHAR : prefix; // Default namespace is ""
			...
			case XMLStreamConstants.SPACE:
				text = _xmlReader.getText();
				_contentHandler.ignorableWhitespace(text.array(), text.offset(),
						text.length());
				break;
			case XMLStreamConstants.PROCESSING_INSTRUCTION:
				_contentHandler.processingInstruction(_xmlReader.getPITarget(),
						_xmlReader.getPIData());
				break;
