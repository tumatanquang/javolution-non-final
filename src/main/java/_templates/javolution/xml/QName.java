 ...
 * @see <a href="//en.wikipedia.org/wiki/Qname">Wikipedia: QName</a>
 */
public final class QName implements XMLSerializable, Immutable, CharSequence {
	...
	private QName(String namespaceURI, String localName, String toString) {
		_namespaceURI = namespaceURI == null ? null
				: new CharArray(namespaceURI);
	...
	public static QName valueOf(CharSequence name) {
		final QName qName = (QName) FULL_NAME_TO_QNAME.get(name);
	...
	private static QName createNoNamespace(String name) {
		...
		final QName qName = new QName(namespaceURI, localName, name);
		synchronized(FULL_NAME_TO_QNAME) {
			final QName tmp = (QName) FULL_NAME_TO_QNAME.putIfAbsent(name,
					qName);
