...
import _templates.javolution.io.UTF8StreamReader;
...
public final class XMLStreamReaderImpl implements XMLStreamReader, Reusable {
	 ...
	 */
	private static final String[] NAMES_OF_EVENTS = new String[] {"UNDEFINED",
	 ...
	 */
	private static final int READER_BUFFER_CAPACITY = 4 << 10;
	 ...
	 */
	private boolean _charactersPending;
	/**
	...
	public void setInput(InputStream in) throws XMLStreamException {
		...
		final CharArray prologEncoding = getCharacterEncodingScheme();
		...
		if(prologEncoding != null && !prologEncoding.equals(_encoding)
			...
			final int startOffset = _readCount;
			reset();
			_startOffset = startOffset;
			setInput(in, prologEncoding.toString());
		}
	}
	private static final boolean isUTF8(Object encoding) {
	 ...
	 * @see    _templates.javolution.io.UTF8StreamReader
	 * @see    _templates.javolution.io.UTF8ByteBufferReader
	 * @see    _templates.javolution.io.CharSequenceReader
	 */
	public void setInput(Reader reader) throws XMLStreamException {
		...
		try { // Reads prolog (if there)
			final int readCount = reader.read(_readBuffer, _startOffset,
					_readBuffer.length - _startOffset);
			_readCount = readCount >= 0 ? readCount + _startOffset
					: _startOffset;
			if(_readCount >= 5 && _readBuffer[0] == '<' && _readBuffer[1] == '?'
					&& _readBuffer[2] == 'x' && _readBuffer[3] == 'm'
					&& _readBuffer[4] == 'l' && _readBuffer[5] == ' ') { // Prolog detected.
	 ...
	 * reached. The depth is decremented AFTER the end tag event was observed.[code]
	 * <!-- outside -->      0
	 * <root>                1
	 *     sometext          1
	 *     <foobar>          2
	 ...
	 * reader.setEntities(HTML_ENTITIES);[/code]
	 *
	...
	public int next() throws XMLStreamException {
		...
		case END_ELEMENT:
			_namespaces.pop();
			final CharArray startElem = _elemStack[_depth--];
		...
		for(;;) {
			...
			case STATE_CHARACTERS:
				for(;;) { // Read characters data all at once.
					if(c == '<') {
						final int length = _index - _start - 1;
						if(length > 0) {
				...
				break;
			case STATE_CDATA:
				for(;;) { // Reads CDATA all at once.
					if(c == '>' && _index - _start >= 3
							&& _data[_index - 2] == ']'
							&& _data[_index - 3] == ']') {
						_index -= 3;
						final int length = _index - _start;
			...
			case STATE_MARKUP: // Starts with '<'
				if(_index - _start == 2) {
					...
					if(_charactersPending) { // Flush character event.
						...
						&& _data[_start + 2] == '[' && _data[_start + 3] == 'C'
						&& _data[_start + 4] == 'D' && _data[_start + 5] == 'A'
						&& _data[_start + 6] == 'T' && _data[_start + 7] == 'A'
						&& _data[_start + 8] == '[') {
					_start = _index = _index - 9; // Do not keep <![CDATA[
					_state = STATE_CDATA;
				}
				else if(_index - _start == 9 && _data[_start + 1] == '!'
						&& _data[_start + 2] == 'D' && _data[_start + 3] == 'O'
						&& _data[_start + 4] == 'C' && _data[_start + 5] == 'T'
						&& _data[_start + 6] == 'Y' && _data[_start + 7] == 'P'
						&& _data[_start + 8] == 'E') {
			...
			case STATE_OPEN_TAGxREAD_ELEM_NAME:
				for(;;) { // Read element name all at once.
			...
			case STATE_OPEN_TAGxREAD_ATTR_NAME:
				for(;;) { // Read attribute name all at once.
					if(c < '@') { // Else avoid multiple checks.
						if(c <= ' ') {
			...
			case STATE_OPEN_TAGxREAD_ATTR_VALUE_SIMPLE_QUOTE:
				for(;;) { // Read attribute value all at once.
					if(c == '\'') {
			...
			case STATE_CLOSE_TAGxREAD_ELEM_NAME:
				for(;;) { // Element name can be read all at once.
					if(c < '@') { // Else avoid multiple checks.
						if(c == '>') {
							_qName = newSeq(_start, --_index - _start);
							_start = _index;
	 ...
	 */
	private final void reloadBuffer() throws XMLStreamException {
	 ...
	 */
	private final boolean isEndOfStream() throws XMLStreamException {
		if(_readIndex >= _readCount) {
			reloadBuffer();
		}
		if(_readCount <= 0) {
	 ...
	 */
	private final char handleEndOfLine(char c) throws XMLStreamException {
		switch(c) {
		case 0xD: // Replaces #xD with #xA
	 ...
	 */
	private final char replaceEntity() throws XMLStreamException {
		if(_state == STATE_COMMENT || _state == STATE_PI
				|| _state == STATE_CDATA)
			return '&'; // (&2.4)
		final int start = _index; // Index of first replacement character.
		_data[_index++] = '&';
		for(;;) {
			if(_readIndex >= _readCount) {
				reloadBuffer();
			}
			final char c = _data[_index++] = _readBuffer[_readIndex++];
			if(c == ';') {
				break;
			}
			if(c <= ' ')
		...
		// Replaces the entity.
		final int length = _entities.replaceEntity(_data, start,
				_index - start);
	 ...
	 */
	private final void processAttribute() throws XMLStreamException {
		if(_attrPrefixSep < 0) { // No prefix.
			...
			final CharArray prefix = newSeq(offset, _attrPrefixSep - offset);
			final CharArray localName = newSeq(_attrPrefixSep + 1,
					offset + length - _attrPrefixSep - 1);
	...
	private final CharArray newSeq(int offset, int length) {
		final CharArray seq = _seqsIndex < _seqsCapacity ? _seqs[_seqsIndex++]
				: newSeq2();
		return seq.setArray(_data, offset, length);
	}
	private final CharArray newSeq2() {
	...
	private final Runnable _createSeqLogic = new Runnable() {
		public final void run() {
			if(_seqsCapacity >= _seqs.length) { // Resizes.
				final CharArray[] tmp = new CharArray[_seqs.length * 2];
				System.arraycopy(_seqs, 0, tmp, 0, _seqs.length);
				_seqs = tmp;
			}
			final CharArray seq = new CharArray();
	...
	private final void increaseDataBuffer() {
		...
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
				final char[] tmp = new char[_data.length << 1];
	...
	private final void increaseStack() {
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
				final CharArray[] tmp = new CharArray[_elemStack.length << 1];
				LogContext.info(new CharArray(
	...
	public void require(int type, CharSequence namespaceURI,
		...
		if(_eventType != type)
			throw new XMLStreamException(
					"Expected event: " + NAMES_OF_EVENTS[type]
							+ ", found event: " + NAMES_OF_EVENTS[_eventType]);
		...
		while(eventType != XMLStreamConstants.END_ELEMENT) {
			...
			case XMLStreamConstants.CHARACTERS:
				if(text == null) {
					...
					text.setArray(_data, text.offset(),
			...
			default:
				throw new XMLStreamException(
						"Unexpected event type " + NAMES_OF_EVENTS[eventType],
						getLocation());
			}
			eventType = next();
		}
	...
	public CharArray getAttributeNamespace(int index) {
		if(_eventType != XMLStreamConstants.START_ELEMENT)
			throw illegalState("Not a start element");
		final CharArray prefix = _attributes.getPrefix(index);
	...
	public CharArray getAttributeValue(CharSequence uri,
		...
		if(_eventType != XMLStreamConstants.START_ELEMENT)
			throw illegalState("Not a start element");
		return uri == null ? _attributes.getValue(localName)
				: _attributes.getValue(uri, localName);
	}
	...
	public CharArray getLocalName() {
		...
		if(_prefixSep < 0)
			return _qName;
		final CharArray localName = newSeq(_prefixSep + 1,
	...
	public CharArray getPrefix() {
		...
		if(_prefixSep < 0)
			return null;
		final int offset = _qName.offset();
		final CharArray prefix = newSeq(offset, _prefixSep - offset);
		return prefix;
	}
	public CharArray getPIData() {
		...
		final int offset = _text.indexOf(' ') + _text.offset() + 1;
		final CharArray piData = newSeq(offset, _text.length() - offset);
		return piData;
	}
	public CharArray getPITarget() {
		if(_eventType != XMLStreamConstants.PROCESSING_INSTRUCTION)
			throw illegalState("Not a processing instruction");
		final CharArray piTarget = newSeq(_text.offset(),
	...
	public int getTextCharacters(int sourceStart, char[] target,
		...
		final CharArray text = getText();
		final int copyLength = Math.min(length, text.length());
	...
	public boolean isStandalone() {
		final CharArray standalone = readPrologAttribute(STANDALONE);
	...
	public boolean hasText() {
		return (_eventType == XMLStreamConstants.CHARACTERS
				|| _eventType == XMLStreamConstants.COMMENT
				|| _eventType == XMLStreamConstants.DTD) && _text.length() > 0;
	}
	...
	public boolean isWhiteSpace() {
		if(isCharacters()) {
			final char[] chars = _text.array();
			for(int i = _text.offset(), end = _text.offset()
					+ _text.length(); i < end;) {
	...
	private final IllegalStateException illegalState(String msg) {
		return new IllegalStateException(
				msg + " (" + NAMES_OF_EVENTS[_eventType] + ')');
	}
	private final String detectEncoding(InputStream input)
			throws XMLStreamException {
	...
	private static final int READ_EQUAL = 0;
	private static final int READ_QUOTE = 1;
	private static final int VALUE_SIMPLE_QUOTE = 2;
	private static final int VALUE_DOUBLE_QUOTE = 3;
	private final CharArray readPrologAttribute(CharSequence name) {
		...
		if(i >= 0) {
			i += _prolog.offset();
			final int maxIndex = _prolog.offset() + _prolog.length();
			i += name.length();
			int state = READ_EQUAL;
			int valueOffset = 0;
			while(i < maxIndex) {
				final char c = _prolog.array()[i++];
				switch(state) {
				case READ_EQUAL:
					if(c == '=') {
						state = READ_QUOTE;
