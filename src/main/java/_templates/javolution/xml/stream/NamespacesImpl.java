...
final class NamespacesImpl implements Reusable, NamespaceContext {
	...
	public CharArray getPrefix(CharSequence uri) {
		...
		return _defaultNamespace.equals(uri) ? _defaultNsPrefix
				: getPrefix(uri, _namespacesCount[_nesting]);
	}
	CharArray getPrefix(CharSequence uri, int count) {
		for(int i = count; --i >= 0;) {
			final CharArray prefix = _prefixes[i];
			final CharArray namespace = _namespaces[i];
	...
	public Iterator getPrefixes(CharSequence namespaceURI) {
		final FastList prefixes = new FastList();
			...
			MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
				public final void run() {
					_prefixesTmp[index] = new CharArray()
							.setArray(new char[prefixLength + 32], 0, 0);
				}
			});
			...
			MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
				public final void run() {
					_namespacesTmp[index] = new CharArray()
							.setArray(new char[uriLength + 32], 0, 0);
				}
			});
	...
	private final void searchDefaultNamespace() {
		final int count = _namespacesCount[_nesting];
		for(int i = count; --i >= 0;) {
	...
	private final void resizeNamespacesCount() {
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
	...
	private final void resizePrefixStack() {
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
				final int oldLength = _prefixes.length;
				final int newLength = oldLength * 2;
				// Resizes prefixes.
				final CharArray[] tmp0 = new CharArray[newLength];
				System.arraycopy(_prefixes, 0, tmp0, 0, oldLength);
				_prefixes = tmp0;
				// Resizes namespaces uri.
				final CharArray[] tmp1 = new CharArray[newLength];
				System.arraycopy(_namespaces, 0, tmp1, 0, oldLength);
				_namespaces = tmp1;
				// Resizes prefix sets.
				final boolean[] tmp2 = new boolean[newLength];
