...
public final class XMLStreamWriterImpl implements XMLStreamWriter, Reusable {
	 ...
	 */
	private static final int BUFFER_LENGTH = 2 << 10;
	/**
	 ...
	 * @see    _templates.javolution.io.UTF8StreamWriter
	 * @see    _templates.javolution.io.UTF8ByteBufferWriter
	 * @see    _templates.javolution.io.AppendableWriter
	 */
	...
	public void setPrefix(CharSequence prefix, CharSequence uri)
			throws XMLStreamException {
		_namespaces.setPrefix(prefix,
				uri == null ? _namespaces._nullNsURI : uri, false);
	}
	// Implements XMLStreamWriter interface.
	public void setDefaultNamespace(CharSequence uri)
			throws XMLStreamException {
	...
	private void writeNewElement(CharSequence prefix, CharSequence localName,
		...
		_namespaces.push();
		// Constructs qName.
		final TextBuilder qName = _qNames[_nesting].clear();
	...
	private void writeNamespaces() throws XMLStreamException {
		final int i0 = _nesting > 1 ? _namespaces._namespacesCount[_nesting - 2]
				: NamespacesImpl.NBR_PREDEFINED_NAMESPACES;
		final int i1 = _namespaces._namespacesCount[_nesting - 1];
		final int i2 = _namespaces._namespacesCount[_nesting];
		for(int i = i0; i < i2; i++) {
			if(_isRepairingNamespaces && i < i1
					&& !_namespaces._prefixesWritten[i]
					|| i >= i1 && _namespaces._prefixesWritten[i]) { // Write namespace.
				// In repairing mode, removes redondancy.
				if(_isRepairingNamespaces) {
					final CharArray prefix = _namespaces
							.getPrefix(_namespaces._namespaces[i], i);
	...
	private CharSequence getRepairedPrefix(CharSequence prefix,
			CharSequence namespaceURI) throws XMLStreamException {
		final CharArray prefixForURI = _namespaces.getPrefix(namespaceURI);
	...
	private void resizeElemStack() {
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public final void run() {
				...
				final TextBuilder[] tmp = new TextBuilder[newLength];
	...
	private final void write(Object csq, int start, int length,
		...
		if(_index + length <= BUFFER_LENGTH) { // Enough buffer space.
			...
			else if(csq instanceof _templates.javolution.text.Text) {
				((_templates.javolution.text.Text) csq).getChars(start,
						start + length, _buffer, _index);
			}
			else if(csq instanceof _templates.javolution.text.TextBuilder) {
				((_templates.javolution.text.TextBuilder) csq).getChars(start,
						start + length, _buffer, _index);
			}
			else if(csq instanceof _templates.javolution.text.CharArray) {
				((_templates.javolution.text.CharArray) csq).getChars(start,
						start + length, _buffer, _index);
			}
			else {
			...
			if(escapeMarkup) {
				final int end = _index + length;
				for(int i = _index; i < end; ++i) {
					final char c = _buffer[i];
					if(c >= '?' || !isEscaped(c)) {
						continue;
					}
	...
	private final void writeDirectEscapedCharacters(char[] chars, int start,
		...
		try {
			...
			for(int i = start; i < end;) {
				...
				final int blockLength = i - blockStart - 1;
				if(blockLength > 0) {
			...
			final int blockLength = end - blockStart;
			if(blockLength > 0) {
				_writer.write(_buffer, blockStart, blockLength);
			}
		}
	...
	private boolean isEscaped(char c) {
		return c < ' ' && _isAttributeValue || c == '"' && _isAttributeValue
				|| c == '<' || c == '>' || c == '&';
	}
