...
final class EntitiesImpl implements Reusable {
	...
	public int replaceEntity(char[] buffer, int start, int length)
		...
		if(buffer[start + 1] == '#') {
			...
			final int base = c == 'x' ? 16 : 10;
			int i = c == 'x' ? 3 : 2;
			int charValue = 0;
			for(final int len = length - 1; i < len; ++i) {
				c = buffer[start + i];
				charValue *= base;
				charValue += c <= '9' ? c - '0' : c <= 'Z' ? c - 'A' : c - 'a';
			}
		...
		final CharSequence replacementText = _entitiesMapping != null
				? (CharSequence) _entitiesMapping.get(_tmp)
				: null;
		if(replacementText == null)
			throw new XMLStreamException("Entity " + _tmp + " not recognized");
		final int replacementTextLength = replacementText.length();
		for(int i = 0; i < replacementTextLength; ++i) {
			buffer[start + i] = replacementText.charAt(i);
		}
		return replacementTextLength;
	}
	private final CharArray _tmp = new CharArray();
	/**
	...
	public void setEntitiesMapping(Map entityToReplacementText) {
		...
		final Collection values = entityToReplacementText.values();
		if(values instanceof FastCollection) { // Avoids allocating iterators.
			final FastCollection fc = (FastCollection) values;
			for(Record r = fc.head(), t = fc.tail(); (r = r.getNext()) != t;) {
				final CharSequence value = (CharSequence) fc.valueOf(r);
			...
			for(final Iterator i = values.iterator(); i.hasNext();) {
				final CharSequence value = (CharSequence) i.next();
				if(_maxLength < value.length()) {
					_maxLength = value.length();
				}
			}
