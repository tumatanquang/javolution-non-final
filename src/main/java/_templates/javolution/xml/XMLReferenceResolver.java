...
public class XMLReferenceResolver implements Reusable {
	 ...
	 */
	private final FastTable _idToObject = new FastTable();
	/**
	...
	private final TextBuilder _tmp = new TextBuilder();
	/**
	...
	public Object readReference(XMLFormat.InputElement xml)
			throws XMLStreamException {
		final CharArray value = xml._reader.getAttributeValue(toCsq(_refURI),
				toCsq(_refName));
		if(value == null)
			return null;
		final int ref = value.toInt();
	...
	public void createReference(Object obj, XMLFormat.InputElement xml)
			throws XMLStreamException {
		final CharArray value = xml._reader.getAttributeValue(toCsq(_idURI),
				toCsq(_idName));
		if(value == null)
			return;
