...
public abstract class TestContext extends LogContext {
	 ...
	 */
	public static final Configurable/*<Class<? extends TestContext>>*/ DEFAULT = new Configurable(
			Default.class) {};
	/**
	...
	public static void run(TestSuite testSuite) throws Exception {
		final TestContext testContext = (TestContext) LogContext
	...
	public static boolean assertEquals(Object expected, Object actual,
		...
		final boolean ok = expected == null && actual == null
				|| expected != null && expected.equals(actual);
		message = ok || message != null ? message : // Provides error message if necessary.
				Text.valueOf(expected).plus(" expected but found ")
						.plus(actual);
		final TestContext ctx = (TestContext) LogContext.getCurrentLogContext();
	...
	public static boolean assertSame(Object expected, Object actual,
		...
		message = ok || message != null ? message : // Provides error message if necessary.
				Text.valueOf(expected)
						.plus(" expected but found a different instance ")
						.plus(actual);
		final TestContext ctx = (TestContext) LogContext.getCurrentLogContext();
		return ctx.doAssert(ok, message);
	}
	...
	public static boolean assertTrue(boolean actual, CharSequence message) {
		return TestContext.assertEquals(Boolean.TRUE,
				actual ? Boolean.TRUE : Boolean.FALSE, message);
	}
	/**
	...
	public static boolean assertFalse(boolean actual, CharSequence message) {
		return TestContext.assertEquals(Boolean.FALSE,
				actual ? Boolean.TRUE : Boolean.FALSE, message);
	}
	/**
	 ...
	 */
	public static boolean assertEquals(double expected, double actual,
			double delta, CharSequence message) {
		final boolean ok = expected == actual
				|| Double.isNaN(expected) && Double.isNaN(actual);
		message = ok || message != null ? message : // Provides error message if necessary.
				Text.valueOf(expected + " expected but found " + actual);
	...
	public static boolean assertArrayEquals(Object[] expected, Object[] actual,
		...
		if(expected == null)
			return ctx.doAssert(false, message != null ? message
					: Text.valueOf(
							"Null array expected but found actual array not null"));
		if(actual == null)
			return ctx.doAssert(false, message != null ? message
					: Text.valueOf(
							"Not null array expected but found actual array null"));
		if(expected.length != actual.length)
			return ctx.doAssert(false, message != null ? message
					: Text.valueOf("Array of size " + expected.length
							+ " expected but found array of actual size "
							+ actual.length));
				...
				return ctx
						.doAssert(false,
								message != null ? message
										: Text.valueOf("Array element at " + i
												+ ", expected " + e
												+ " but found " + a));
		}
		return ctx.doAssert(true, message);
	...
	public static boolean assertArrayEquals(boolean[] expected,
		...
		for(int i = 0; i < expected.length; ++i) {
			final boolean e = expected[i];
			final boolean a = actual[i];
			if(e != a)
				return ctx
						.doAssert(false,
								message != null ? message
	...
	public static boolean assertArrayEquals(long[] expected, long[] actual,
		...
		final int len = expected.length;
		for(int i = 0; i < len; ++i) {
			final long e = expected[i];
			final long a = actual[i];
			if(e != a)
				return ctx
						.doAssert(false,
								message != null ? message
	 ...
	 */
	public static boolean assertArrayEquals(double[] expected, double[] actual,
			double delta, final CharSequence message) {
		final TestContext ctx = (TestContext) LogContext.getCurrentLogContext();
		if(expected == actual)
			return ctx.doAssert(true, message);
		if(expected == null)
	...
	public static boolean assertException(Class exceptionClass, Runnable logic,
		...
		final TestContext ctx = (TestContext) LogContext.getCurrentLogContext();
		if(exception == null)
			return ctx.doAssert(false,
					message != null ? message
							: Text.valueOf("Expected exception instance of ")
									.plus(exceptionClass.getName())
									.plus(" but no exception has been raised"));
		final boolean ok = exceptionClass.isInstance(exception);
		message = ok || message != null ? message : // Provides error message if necessary.
				Text.valueOf("Expected instance of ")
						.plus(exceptionClass.getName())
						.plus(" but actual exception is instance of ")
						.plus(exception.getClass().getName());
		return ctx.doAssert(ok, message);
	}
	/**
	 * Convenience method equivalent to
	 ...
	 */
	public static boolean assertException(Class exceptionClass,
			Runnable logic) {
	...
	protected void doRun(TestSuite testSuite) throws Exception {
		testSuite.setUp();
		try {
			final FastTable tests = testSuite._tests;
	 ...
	 *         at javolution.TextTestSuite$DoubleParseDouble.validate(TextTestSuite.java:389)[/code]
	 *
	...
	protected boolean doAssert(boolean value, CharSequence message) {
		if(value)
			return true;
		/*@JVM-1.4+@
		final Throwable error = new Error(); // To get stack trace.
		final StackTraceElement[] trace = error.getStackTrace();
		final javolution.text.TextBuilder tmp = javolution.text.TextBuilder.newInstance();
		try {
		    tmp.append(message);
	...
	private static class Default extends TestContext {
		...
		protected void exitAction() {
			logMessage("test", Text.valueOf(
					"---------------------------------------------------"));
			logMessage("test",
					Text.valueOf("SUMMARY - PASSED: " + _passedCount
							+ ", FAILED: " + _failedCount + ", IGNORED: "
							+ _ignoredCount));
		}
		protected void doRun(TestSuite testSuite) throws Exception {
			logMessage("test", Text.valueOf(
					"---------------------------------------------------"));
			logMessage("test", Text.valueOf("Executes Test Suite: ")
					.plus(testSuite.getName()));
			logMessage("test", Text.valueOf(""));
	...
	private static final class Console extends Default {
		/*@JVM-1.6+@
