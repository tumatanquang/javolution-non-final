...
public abstract class TimeContext extends TestContext {
	 ...
	 */
	public static final Configurable/*<Integer>*/ TEST_DURATION_MS = new Configurable(
			new Integer(1000)) {};
	/**
	 * Holds the time context default implementation (by default logs
	 * average and minimum execution time to <code>System.out</code>).
	 */
	public static final Configurable/*<Class<? extends TimeContext>>*/ DEFAULT = new Configurable(
			Default.class) {};
	/**
	...
	public static long getMinimumTime(String unit) {
		final LogContext ctx = LogContext.getCurrentLogContext();
		if(ctx instanceof TimeContext)
			return TimeContext.picosecondTo(unit,
					((TimeContext) ctx).getMinimumTimeInPicoSeconds());
		return -1;
	}
	/**
	...
	public static long getAverageTime(String unit) {
		...
		if(ctx instanceof TimeContext)
			return TimeContext.picosecondTo(unit,
					((TimeContext) ctx).getAverageTimeInPicoSeconds());
		return -1;
	}
	/**
	...
	public static long getMaximumTime(String unit) {
		...
		if(ctx instanceof TimeContext)
			return TimeContext.picosecondTo(unit,
					((TimeContext) ctx).getMaximumTimeInPicoSeconds());
		return -1;
	}
	...
	protected void doRun(TestCase testCase) throws Exception {
		...
		final long maximumDurationPs = ((Integer) TEST_DURATION_MS.get())
				.intValue() * 1000000000L;
		do {
			testCase.setUp(); // Prepare
			try {
				final long start = TimeContext.nanoTime();
				testCase.execute(); // Execute.
				final long duration = (TimeContext.nanoTime() - start) * 1000; // Picoseconds
				final int count = testCase.count();
				totalCount += count;
				totalDuration += duration;
				final long singleExecutionDuration = duration / count;
	...
	private static final class Default extends TimeContext {
		...
		protected void doRun(TestCase testCase) {
			...
			try { // Formats time report.
				...
				tmp.setLength(40, ' ');
				tmp.append(" - Average: ");
				Default.appendTime(getAverageTimeInPicoSeconds(), tmp);
				tmp.append(", Minimum: ");
				Default.appendTime(getMinimumTimeInPicoSeconds(), tmp);
				tmp.append(", Maximum: ");
				Default.appendTime(getMaximumTimeInPicoSeconds(), tmp);
				logMessage("time", tmp);
			}
			finally {
		...
		private static TextBuilder appendTime(long picoseconds,
				TextBuilder tb) {
			long divisor;
			String unit;
			...
			final long value = picoseconds / divisor;
			tb.append(value);
			final int fracDigits = 3 - MathLib.digitLength(value); // 3 digits
			// precision
			if(fracDigits > 0) {
				tb.append(".");
			}
