...
import _templates.javolution.lang.ValueType;
 ...
 *     <a href="//java.sun.com/j2se/1.5.0/docs/api/java/lang/String.html">
 *     Java String</a> and
 *     <a href="//msdn2.microsoft.com/en-us/library/system.string.aspx">
 ...
 *         href="//bugs.sun.com/bugdatabase/view_bug.do?bug_id=4513622">
 ...
 *         {@link _templates.javolution.context.StackContext StackContext}).</li>
 *     </ul></p>
 ...
 * final static Text FALSE = Text.intern("false");[/code]
 ...
 * FastTable myTable ...;
 * myTable.toText().println(); // Prints to System.out[/code]</p>
 *
 ...
 *       href="//en.wikipedia.org/wiki/Tree_rotation">tree rotations</a>.
...
public final class Text implements CharSequence, Comparable, XMLSerializable,
	...
	public Text(String str) {
		...
		if(_data != null) { // Primitive.
			...
			final int half = _count + BLOCK_SIZE >> 1 & BLOCK_MASK;
	...
	private static Text valueOf(String str, int start, int end) {
		...
		if(length <= BLOCK_SIZE) {
			final Text text = newPrimitive(length);
			str.getChars(start, end, text._data, 0);
			return text;
		}
		// Splits on a block boundary.
		final int half = length + BLOCK_SIZE >> 1 & BLOCK_MASK;
	...
	public static Text valueOf(char[] chars, int offset, int length) {
		...
		if(length <= BLOCK_SIZE) {
			final Text text = Text.newPrimitive(length);
	...
	public static Text valueOf(char c) {
		final Text text = Text.newPrimitive(1);
		text._data[0] = c;
		return text;
	}
	/**
	...
	public Text plus(Object obj) {
		return concat(Text.valueOf(obj));
	}
	/**
	...
	public Text plus(String str) {
		final Text merge = append(str);
		return merge != null ? merge : concat(Text.valueOf(str));
	}
	private final Text append(String str) { // Try to append, returns null if cannot.
		final int length = str.length();
		if(_data == null) {
			final Text merge = _tail.append(str);
			return merge != null ? Text.newComposite(_head, merge) : null;
		}
		// Primitive.
		if(_count + length > BLOCK_SIZE)
			return null; // Cannot merge.
		final Text text = Text.newPrimitive(_count + length);
	...
	public Text concat(Text that) {
		...
		final int length = _count + that._count;
		if(length <= BLOCK_SIZE) { // Merges to primitive.
			final Text text = Text.newPrimitive(length);
			getChars(0, _count, text._data, 0);
			that.getChars(0, that._count, text._data, _count);
			return text;
		}
		// Returns a composite.
		Text head = this;
	...
	public Text replace(CharSequence target, CharSequence replacement) {
		final int i = indexOf(target);
		return i < 0 ? this : // No target sequence found.
				subtext(0, i).concat(Text.valueOf(replacement))
						.concat(subtext(i + target.length()).replace(target,
								replacement));
	}
	/**
	...
	public Text replace(CharSet charSet, CharSequence replacement) {
		final int i = indexOfAny(charSet);
		return i < 0 ? this : // No character to replace.
				subtext(0, i).concat(Text.valueOf(replacement))
						.concat(subtext(i + 1).replace(charSet, replacement));
	}
	/**
	...
	public int indexOf(CharSequence csq, int fromIndex) {
		...
		final int min = MathLib.max(0, fromIndex);
		final int max = _count - csqLength;
		if(csqLength == 0)
			return min > max ? -1 : min;
		// Searches for csq.
		final char c = csq.charAt(0);
		for(int i = indexOf(c, min); i >= 0 && i <= max; i = indexOf(c, ++i)) {
			boolean match = true;
	...
	public static Text intern(CharSequence csq) {
		final Text text = (Text) INTERN_INSTANCES.get(csq); // Thread-Safe - No entry removed.
	...
	public static Text intern(String str) {
		final Text text = (Text) INTERN_INSTANCES.get(str); // Thread-Safe - No entry removed.
	...
	private static synchronized Text internImpl(final String str) {
		if(!INTERN_INSTANCES.containsKey(str)) { // Synchronized check.
			MemoryArea.getMemoryArea(INTERN_INSTANCES)
	...
	public boolean contentEqualsIgnoreCase(CharSequence csq) {
		...
		for(int i = 0; i < _count;) {
			...
			if(u1 != u2) {
				...
				if(u1 != u2 && Character.toLowerCase(u1) != Character
						.toLowerCase(u2))
					return false;
			}
		}
	 ...
	 * {@link _templates.javolution.lang.ValueType Realtime} interface).
	 *
	...
	public void printStatistics(PrintStream out) {
		final int length = length();
		final int leaves = getNbrOfLeaves();
		synchronized(out) {
			...
			out.print(
					", AVG LEAVE LENGTH: " + (length + (leaves >> 1)) / leaves);
			out.println();
		}
	}
	...
	private final int getNbrOfBranches() {
		return _data == null
				? _head.getNbrOfBranches() + _tail.getNbrOfBranches() + 1
				: 0;
	}
	private final int getNbrOfLeaves() {
		return _data == null ? _head.getNbrOfLeaves() + _tail.getNbrOfLeaves()
				: 1;
	}
	/**
	 ...
	 * }
	 * ...
	 * static final OUT = new UTF8StreamWriter().setOutput(System.out);[/code]
	 */
	public void print() {
		try {
	...
	public Text toLowerCase() {
		...
		final Text text = Text.newPrimitive(_count);
		for(int i = 0; i < _count;) {
	...
	public char charAt(int index) {
		if(index >= _count)
			throw new IndexOutOfBoundsException();
		return _data != null ? _data[index]
				: index < _head._count ? _head.charAt(index)
						: _tail.charAt(index - _head._count);
	}
	/**
	...
	public Text subtext(int start, int end) {
		...
		return _head.subtext(start, cesure)
				.concat(_tail.subtext(0, end - cesure));
	}
	/**
	...
	public String toString() {
		...
		// Composite.
		final char[] data = new char[_count];
		getChars(0, _count, data, 0);
	...
	public Text trimStart() {
		int first = 0; // First character index.
		final int last = length() - 1; // Last character index.
		while(first <= last && charAt(first) <= ' ') {
			first++;
		}
		return subtext(first, last + 1);
	 ...
	 */
	public Text trimEnd() {
		final int first = 0; // First character index.
		int last = length() - 1; // Last character index.
		while(last >= first && charAt(last) <= ' ') {
			last--;
		}
	...
	////////////////////////////////////////////////////////////////////////////
	/**
	 * Returns a {@link _templates.javolution.context.AllocatorContext context allocated}
	 * primitive text instance.
	 *
	 * @param length the primitive length.
	 */
	private static final Text newPrimitive(int length) {
		final Text text = (Text) PRIMITIVE_FACTORY.object();
		text._count = length;
		return text;
	}
	/**
	 ...
	 */
	private static final Text newComposite(Text head, Text tail) {
		final Text text = (Text) COMPOSITE_FACTORY.object();
