 ...
 *     class to define the default textual representation of its instances.[code]
 ...
 *
 * }[/code]
 *     The format associated to any given class/object can be dynamically retrieved.[code]
 ...
 *     }
 * }[/code]
 *     The default format can be locally overriden.[code]
 * LocalContext.enter();
 * try {
...
public abstract class TextFormat/*<T>*/ {
	...
	protected TextFormat(Class/*<T>*/ forClass) {
		...
		Reflection.getInstance().setField(new LocalReference(this), forClass,
	 ...
	 */
	public static/*<T>*/ TextFormat/*<T>*/ getDefault(
			Class/*<? extends T>*/ forClass) {
		Predefined.init(); // Forces initialization.
		final LocalReference localReference = (LocalReference) Reflection
				.getInstance().getField(forClass, LocalReference.class, true);
		return localReference == null ? Predefined.OBJECT_FORMAT
				: (TextFormat/*<T>*/) localReference.getDefault();
	}
	/**
	 ...
	 * @return the most specialized format compatible with the specified class.
	 */
	public static/*<T>*/ TextFormat/*<T>*/ getInstance(
			Class/*<? extends T>*/ forClass) {
		Predefined.init(); // Forces initialization.
		final LocalReference localReference = (LocalReference) Reflection
				.getInstance().getField(forClass, LocalReference.class, true);
		return localReference == null ? Predefined.OBJECT_FORMAT
				: (TextFormat/*<T>*/) localReference.get();
	}
	/**
	...
	public static/*<T>*/ void setInstance(Class/*<? extends T>*/ forClass,
		...
		Predefined.init(); // Forces initialization.
		final LocalReference localReference = (LocalReference) Reflection
				.getInstance().getField(forClass, LocalReference.class, false);
		if(localReference == null)
			throw new IllegalArgumentException(
					"Cannot override default format for class " + forClass
							+ " (no default format defined)");
	...
	public final Object/*{T}*/ parse(CharSequence csq)
			throws IllegalArgumentException {
		final Cursor cursor = Cursor.newInstance();
		try {
			final Object/*{T}*/ obj = parse(csq, cursor);
	...
	private static class Predefined {
		...
		private static final TextFormat STRING_FORMAT = new TextFormat(
				String.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
			...
			public Object parse(CharSequence csq, Cursor cursor) {
				final String str = csq
						.subSequence(cursor.getIndex(), csq.length())
						.toString();
				cursor.setIndex(csq.length());
				return str;
			}
		};
		private static final TextFormat BOOLEAN_FORMAT = new TextFormat(
				Boolean.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
			...
			public Object parse(CharSequence csq, Cursor cursor) {
				return TypeFormat.parseBoolean(csq, cursor) ? Boolean.TRUE
						: Boolean.FALSE;
			}
		};
		private static final TextFormat CHARACTER_FORMAT = new TextFormat(
				Character.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat BYTE_FORMAT = new TextFormat(
				Byte.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat SHORT_FORMAT = new TextFormat(
				Short.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat INTEGER_FORMAT = new TextFormat(
				Integer.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat LONG_FORMAT = new TextFormat(
				Long.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat FLOAT_FORMAT = new TextFormat(
				Float.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat DOUBLE_FORMAT = new TextFormat(
				Double.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
		...
		private static final TextFormat CLASS_FORMAT = new TextFormat(
				Class.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
			...
			public Object parse(CharSequence csq, Cursor cursor) {
				final CharSequence className = cursor.nextToken(csq,
						CharSet.WHITESPACES);
				if(className == null)
					throw new IllegalArgumentException("No class name found");
				final Class cls = Reflection.getInstance().getClass(className);
				if(cls != null)
					return cls;
				throw new IllegalArgumentException("Class \"" + className
						+ "\" not found (see javolution.lang.Reflection)");
			}
		};
		private static final TextFormat TEXT_FORMAT = new TextFormat(
				Text.class) {
			public Appendable format(Object obj, Appendable dest)
					throws IOException {
			...
			public Object parse(CharSequence csq, Cursor cursor) {
				final CharSequence subCsq = csq.subSequence(cursor.getIndex(),
						csq.length());
