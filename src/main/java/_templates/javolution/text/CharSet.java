...
public final class CharSet implements Immutable {
	...
	public static final CharSet SPACES = CharSet.valueOf(
			new char[] {0x20, 0xA0, 0x1680, 0x180E, 0x2000, 0x2001, 0x2002,
	 ...
	 */
	public static final CharSet ISO_CONTROLS = CharSet.valueOf(new char[] {0x0,
			0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xA, 0xB, 0xC, 0xD,
			0xE, 0xF, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
			0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x7F, 0x80, 0x81, 0x82,
	...
	public static CharSet valueOf(char[]/*...*/ chars) {
		...
		final CharSet charSet = new CharSet(new long[(maxChar >> 6) + 1]);
		for(int i = chars.length; --i >= 0;) {
	...
	public static CharSet rangeOf(char first, char last) {
		...
		final CharSet charSet = new CharSet(new long[(last >> 6) + 1]);
	...
	/**
	 * Equivalent to
	 * {@link #indexIn(_templates.java.lang.CharSequence, int) indexIn(csq, 0)}
	 *
	...
	/**
	 * Equivalent to
	 * {@link #lastIndexIn(_templates.java.lang.CharSequence, int)
	...
	public CharSet minus(CharSet that) {
		final CharSet result = copy();
		for(int i = MathLib.min(_mapping.length,
				that._mapping.length); --i >= 0;) {
			result._mapping[i] &= ~that._mapping[i];
		}
		return result;
	}
	...
	public String toString() {
		...
		tb.append('{');
		final int len = _mapping.length << 6;
		for(int i = 0; i < len; ++i) {
			if(contains((char) i)) {
				if(tb.length() > 1) {
					tb.append(',');
	...
	private CharSet copy() {
		final CharSet charSet = new CharSet(new long[_mapping.length]);
