 ...
 * TypeFormat.format(0.2, 4, true, true, a) = "2.000E-1" // Scientific notation, fixed number of digits.[/code]</p>
 *
...
public final class TypeFormat {
	...
	public static boolean parseBoolean(CharSequence csq, Cursor cursor) {
		...
		if(end >= start + 5
			...
			if((csq.charAt(++start) == 'a' || csq.charAt(start) == 'A')
				...
				&& (csq.charAt(start) == 't' || csq.charAt(start) == 'T')) {
	...
	public static byte parseByte(CharSequence csq, int radix) {
		final int i = parseInt(csq, radix);
	...
	public static byte parseByte(CharSequence csq, int radix, Cursor cursor) {
		final int i = parseInt(csq, radix, cursor);
		if(i < Byte.MIN_VALUE || i > Byte.MAX_VALUE)
			throw new NumberFormatException("Overflow");
		return (byte) i;
	}
	...
	public static int parseInt(CharSequence csq, int radix, Cursor cursor) {
		...
		for(; i < end; i++) {
			...
			final int digit = c <= '9' ? c - '0'
					: c <= 'Z' && c >= 'A' ? c - 'A' + 10
							: c <= 'z' && c >= 'a' ? c - 'a' + 10 : -1;
			if(digit >= 0 && digit < radix) {
				final int newResult = result * radix - digit;
				if(newResult > result)
					throw new NumberFormatException(
							"Overflow parsing " + csq.subSequence(start, end));
				result = newResult;
			}
			else if(c == '-' && i == start) {
				isNegative = true;
		...
		for(; i < end; ++i) {
			...
			final int digit = c <= '9' ? c - '0'
					: c <= 'Z' && c >= 'A' ? c - 'A' + 10
							: c <= 'z' && c >= 'a' ? c - 'a' + 10 : -1;
			if(digit >= 0 && digit < radix) {
				final long newResult = result * radix - digit;
				if(newResult > result)
					throw new NumberFormatException(
							"Overflow parsing " + csq.subSequence(start, end));
				result = newResult;
	...
	public static double parseDouble(CharSequence csq, Cursor cursor)
		...
		for(;;) {
			final int digit = c - '0';
			if(digit >= 0 && digit < 10) {
				final long tmp = decimal * 10 + digit;
				if(tmp < decimal)
					throw new NumberFormatException(
		...
		final int fractionLength = decimalPoint >= 0 ? i - decimalPoint - 1 : 0;
		// Reads exponent.
		int exp = 0;
		if(i < end && (c == 'E' || c == 'e')) {
			c = csq.charAt(++i);
			final boolean isNegativeExp = c == '-';
	 ...
	 */
	public static Appendable format(boolean b, Appendable a)
			throws IOException {
	...
	private static void increment(Cursor cursor, int inc, int endIndex,
			CharSequence csq) throws NumberFormatException {
		if(cursor != null) {
			cursor.increment(inc);
		}
		else if(inc != endIndex) // Whole string must be parsed.
			throw new NumberFormatException(
					"Extraneous character: '" + csq.charAt(inc) + "'");
	}
	// For J2ME Compatibility.
