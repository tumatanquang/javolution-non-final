...
public class TextBuilder
		implements Appendable, CharSequence, Reusable, Realtime, Serializable {
	/**
	...
	public static TextBuilder newInstance() {
		final TextBuilder textBuilder = (TextBuilder) FACTORY.object();
	...
	public final void getChars(int srcBegin, int srcEnd, char[] dst,
			int dstBegin) {
		if(srcBegin < 0 || srcBegin > srcEnd || srcEnd > _length)
			throw new IndexOutOfBoundsException();
		for(int i = srcBegin, j = dstBegin; i < srcEnd;) {
			final char[] chars0 = _high[i >> B1];
			final int i0 = i & M1;
			final int length = MathLib.min(C1 - i0, srcEnd - i);
	...
	private final TextBuilder appendNumber(Object num) {
	...
	public final TextBuilder append(String str, int start, int end) {
		...
		final int newLength = _length + end - start;
		while(_capacity < newLength) {
			increaseCapacity();
		}
		for(int i = start, j = _length; i < end;) {
			final char[] chars = _high[j >> B1];
			final int dstBegin = j & M1;
			final int inc = MathLib.min(C1 - dstBegin, end - i);
	...
	public final TextBuilder append(char chars[], int offset, int length) {
		...
		if(offset < 0 || length < 0 || end > chars.length)
			throw new IndexOutOfBoundsException();
		final int newLength = _length + length;
		while(_capacity < newLength) {
			increaseCapacity();
		}
		for(int i = offset, j = _length; i < end;) {
			final char[] dstChars = _high[j >> B1];
			final int dstBegin = j & M1;
			final int inc = MathLib.min(C1 - dstBegin, end - i);
	...
	public final TextBuilder append(int i) {
		...
		final int digits = MathLib.digitLength(i);
	...
	private final void appendPositive(int l1, int radix) {
		if(l1 >= radix) {
			final int l2 = l1 / radix;
			// appendPositive(l2, radix);
			if(l2 >= radix) {
				final int l3 = l2 / radix;
				// appendPositive(l3, radix);
				if(l3 >= radix) {
					final int l4 = l3 / radix;
	...
	private void appendPositive(long l1, int radix) {
		if(l1 >= radix) {
			final long l2 = l1 / radix;
			// appendPositive(l2, radix);
			if(l2 >= radix) {
				final long l3 = l2 / radix;
				// appendPositive(l3, radix);
				if(l3 >= radix) {
					final long l4 = l3 / radix;
					appendPositive(l4, radix);
	...
	public final TextBuilder append(float f) {
		return append(f, 10, MathLib.abs(f) >= 1E7 || MathLib.abs(f) < 0.001,
				false);
	}
	/**
	...
	public final TextBuilder append(double d) {
		return append(d, -1, MathLib.abs(d) >= 1E7 || MathLib.abs(d) < 0.001,
				false);
	}
	/**
	 * Appends the textual representation of the specified <code>double</code>
	...
	public final TextBuilder append(double d, int digits, boolean scientific,
		...
		final int e = MathLib.floorLog10(d);
		long m;
		if(digits < 0) { // Use 16 or 17 digits.
			// Try 17 digits.
			final long m17 = MathLib.toLongPow10(d, 17 - 1 - e);
			// Check if we can use 16 digits.
			final long m16 = m17 / 10;
			final double dd = MathLib.toDoublePow10(m16, e - 16 + 1);
		...
		if(scientific || e >= digits) {
			...
			final long pow10 = POW10_LONG[digits - 1];
			final int k = (int) (m / pow10); // Single digit.
			append((char) ('0' + k));
			m = m - pow10 * k;
			...
			final int exp = digits - e - 1;
			if(exp < POW10_LONG.length) {
				final long pow10 = POW10_LONG[exp];
				final long l = m / pow10;
				append(l);
				m = m - pow10 * l;
			}
			else {
	...
	private final void appendFraction(long l, int digits, boolean showZero) {
		...
		if(l == 0) {
			...
			final int length = MathLib.digitLength(l);
	...
	public final String toString() {
		final char[] data = new char[_length];
		getChars(0, _length, data, 0);
	...
	public final CharArray toCharArray() {
		final CharArray cArray = new CharArray();
		char[] data;
		if(_length < C1) {
			data = _low;
		}
	...
	public final boolean equals(Object obj) {
		...
		if(!(obj instanceof TextBuilder))
			return false;
		final TextBuilder that = (TextBuilder) obj;
	...
	public final boolean contentEquals(CharSequence csq) {
		if(csq.length() != _length)
			return false;
		for(int i = 0; i < _length;) {
			final char c = i < C1 ? _low[i] : _high[i >> B1][i & M1];
			if(csq.charAt(i++) != c)
				return false;
		}
		return true;
			...
			public final void run() {
				if(_capacity < C1) { // For small capacity, resize.
					...
					if(j >= _high.length) { // Resizes _high.
						final char[][] tmp = new char[_high.length << 1][];
