...
public abstract class Enum implements Comparable, Serializable {
	...
	protected Enum(String name, int ordinal) {
		...
		synchronized(CLASS_TO_ENUMS) {
			MemoryArea.getMemoryArea(CLASS_TO_ENUMS)
					.executeInArea(new Runnable() {
						public final void run() {
							...
							final Object prev = nameToEnum.put(_name,
									Enum.this);
							if(prev != null)
	...
	public final int compareTo(Object that) {
		final Enum e = (Enum) that;
		if(getClass() == that.getClass())
			return _ordinal - e._ordinal;
	...
	public static Enum valueOf(Class enumType, String name) {
		final FastMap nameToEnum = (FastMap) CLASS_TO_ENUMS.get(enumType);
		if(nameToEnum != null) {
			final Enum e = (Enum) nameToEnum.get(name);
			if(e != null)
				return e;
		}
		throw new IllegalArgumentException(
				enumType + "." + name + " not found");
	}
}