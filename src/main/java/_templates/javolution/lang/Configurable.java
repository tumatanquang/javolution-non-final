...
public class Configurable/*<T>*/ {
	 ...
	 * For example:[code]
	 * Configurable cfg = Configurable.getInstance("javolution.context.ConcurrentContext#MAXIMUM_CONCURRENCY")[/code]
	 * returns {@link javolution.context.ConcurrentContext#MAXIMUM_CONCURRENCY}.
	 *
	...
	public static Configurable getInstance(String name) {
		...
		/*@JVM-1.4+@
		try {
			final String fieldName = name.substring(sep + 1);
	...
	public static /*<T>*/ void configure(Configurable/*<T>*/ cfg,
		...
		if(newValue == null)
			throw new IllegalArgumentException("Default value cannot be null");
		final SecurityContext policy = SecurityContext
				.getCurrentSecurityContext();
		if(!policy.isConfigurable(cfg))
			throw new SecurityException(
					"Configuration disallowed by SecurityContext");
		final Object/*{T}*/ oldValue = cfg._value;
		if(!newValue.equals(oldValue)) {
			LogContext.info(
					"Configurable " + cfg.getName() + " set to " + newValue);
			cfg._value = newValue;
	 ...
	 * javolution.util.FastComparator#REHASH_SYSTEM_HASHCODE = true
	 * javolution.context.ConcurrentContext#MAXIMUM_CONCURRENCY = 0
	 * javolution.xml.stream.XMLInputFactory#CLASS = com.foo.bar.XMLInputFactoryImpl[/code]
	...
	public static void read(java.util.Properties properties) {
		...
		while(e.hasMoreElements()) {
			...
			javolution.text.TextFormat format = javolution.text.TextFormat.getInstance(type);
	 ...
	 * <a href="../../javolution/xml/package-summary.html">
	...
	public static void read(InputStream inputStream) {
		try {
			final XMLObjectReader reader = XMLObjectReader
	...
	private static class ConfigurableXMLFormat extends XMLFormat {
		...
		public void write(Object c, OutputElement xml)
				throws XMLStreamException {
			throw new UnsupportedOperationException();
		}
		public void read(InputElement xml, Object c) throws XMLStreamException {
			final Object value = xml.get("Value");
			if(value == null)
