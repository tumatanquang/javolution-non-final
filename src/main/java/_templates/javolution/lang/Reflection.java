...
/*@JVM-1.1+@import _templates.java.util.Iterator;/**/
 ...
 *     <a href="//www.osgi.org/">OSGi Platform</a>. For example:[code]
...
public abstract class Reflection {
	 ...
	 * Holds the default implementation (configurable).
	 */
	public static final Configurable/*<Class<? extends Reflection>>*/ CLASS = new Configurable(
			Default.class) {
	...
	public Class getClass(String name) {
		...
		final TextBuilder tmp = TextBuilder.newInstance();
		try {
			tmp.append(name);
			return getClass(tmp);
		}
	 ...
	 */
	public abstract /*<T>*/ Object/*{T}*/ getField(Class forClass,
			Class/*<T>*/ type, boolean inherited);
	/**
	 ...
	 */
	public abstract /*<T>*/ void setField(Object/*{T}*/ obj, Class forClass,
			Class/*<T>*/ type);
	/**
	 ...
	 * Object fastMap = fastMapConstructor.newInstance(new Integer(64));[/code]
	 */
	...
	private static final class Default extends Reflection {
		...
		private final FastMap _nameToClass = new FastMap().shared()
		...
		public void remove(Object classLoader) {
			...
			/*@JVM-1.1+@
			for(final Iterator i = _fields.entrySet().iterator(); i.hasNext();) {
				final FastMap.Entry entry = (FastMap.Entry) i.next();
		...
		public Class getClass(CharSequence name) {
			final Class cls = (Class) _nameToClass.get(name); // First search cache.
		...
		private Class searchClass(String name) {
			...
			try {
				...
				/*@JVM-1.4+@
				for(final Iterator i = _classLoaders.iterator(); i.hasNext();) {
					final ClassLoader classLoader = (ClassLoader) i.next();
					try {
		...
		public Constructor getConstructor(String signature) {
			...
			if(argEnd < 0)
				throw new IllegalArgumentException("Parenthesis ')' not found");
			final String className = signature.substring(0, argStart - 1);
			final Class theClass = getClass(className);
			if(theClass == null)
				return null;
			final String args = signature.substring(argStart, argEnd);
			if(args.length() == 0)
				return new DefaultConstructor(theClass);
			/*@JVM-1.4+@
			final Class[] argsTypes = classesFor(args);
			if(argsTypes == null)
				return null;
			try {
			...
			/**/
			LogContext.warning(
					"Reflection not supported (Reflection.getConstructor(String)");
			return null;
		}
		public Class[] getInterfaces(Class cls) {
			...
			/**/
			LogContext.warning(
					"Reflection not supported (Reflection.getInterfaces(Class)");
			return new Class[0];
		}
		public Class getSuperclass(Class cls) {
			...
			/**/
			LogContext.warning(
					"Reflection not supported (Reflection.getSuperclass(Class)");
			return null;
		}
		...
		public Method getMethod(String signature) {
			...
			/**/
			LogContext.warning(
					"Reflection not supported (Reflection.getMethod(String)");
			return null;
		}
		...
		private Object getField2(Class forClass, Class type,
				boolean inherited) {
			final FastMap typeToField = (FastMap) _fields.get(forClass);
			if(typeToField != null) {
				final Object field = typeToField.get(type);
				if(field != null)
					return field;
			}
			if(!inherited)
			...
			final Class[] interfaces = getInterfaces(forClass);
			final int len = interfaces.length;
			for(int i = 0; i < len; ++i) {
				final Object field = getField2(interfaces[i], type, false);
				if(field != null)
					return field;
			}
			// Recursion with the parent class.
			final Class parentClass = getSuperclass(forClass);
			return parentClass != null ? getField2(parentClass, type, inherited)
					: null;
		}
		public void setField(Object obj, Class forClass, Class type) {
			synchronized(forClass) { // We don't want to attach simultaneously to the same class.
				...
				if(typeToField != null && typeToField.containsKey(type))
					throw new IllegalArgumentException("Field of type " + type
							+ " already attached to class " + forClass);
				if(typeToField == null) {
		...
		private static abstract class BaseConstructor implements Constructor {
			...
			public final Object newInstance(Object... args) {
				if(_parameterTypes.length != args.length)
					throw new IllegalArgumentException("Expected number of parameters is " + _parameterTypes.length);
				return allocate(args);
			}
			/**/
		}
			...
			public Object allocate(Object[] args) {
				try {
					...
					throw new RuntimeException(
							"Default constructor instantiation error for "
		...
		private final class ReflectConstructor extends BaseConstructor {
			...
			public Object allocate(Object[] args) {
				try {
					...
					throw new RuntimeException("Instantiation error for " + _signature + " constructor", e);
				}
				catch(IllegalAccessException e) {
					throw new RuntimeException("Illegal access error for " + _signature + " constructor", e);
				}
				catch(java.lang.reflect.InvocationTargetException e) {
					if(e.getCause() instanceof RuntimeException)
						throw (RuntimeException)e.getCause();
					throw new RuntimeException("Invocation exception  for " + _signature + " constructor", (java.lang.reflect.InvocationTargetException) e.getCause());
				}
			}
			public String toString() {
		...
		private final class ReflectMethod extends BaseMethod {
			...
			public Object execute(Object that, Object[] args) {
				try {
					...
					if(e.getCause() instanceof RuntimeException)
						throw (RuntimeException) e.getCause();
					throw new RuntimeException("Invocation exception for " + _signature + " method", (java.lang.reflect.InvocationTargetException) e.getCause());
				}
			}
			public String toString() {
		...
		private Class[] classesFor(String args) {
			...
			for(int i = -1; ++i < length;) {
				classes[i] = classFor(parts[i].trim());
				if(classes[i] == null) {
					return null;
				}
			}
				...
				if(className.indexOf("[][]") >= 0) {
					if(className.indexOf("[][][]") >= 0) {
						if(className.indexOf("[][][][]") >= 0) {
							...
							return getClass("[[["+ descriptorFor(className.substring(0, arrayIndex)));
						}
					}
					else { // Dimension two.
						return getClass("[["+ descriptorFor(className.substring(0, arrayIndex)));
