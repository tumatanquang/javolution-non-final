...
public class ClassInitializer {
	...
	public static void initializeRuntime() {
		...
		if(bootPath == null || pathSeparator == null) {
			LogContext.warning(
					"Cannot initialize boot path through system properties");
			return;
		}
		...
		if(javaHome == null || fileSeparator == null) {
			LogContext.warning(
					"Cannot initialize extension library through system properties");
			return;
		}
		final File extDir = new File(
				javaHome + fileSeparator + "lib" + fileSeparator + "ext");
		if(!extDir.getClass().getName().equals("java.io.File")) {
			LogContext.warning(
					"Extension classes initialization not supported for J2ME build");
			return;
		}
	...
	public static void initializeClassPath() {
		...
		if(classPath == null || pathSeparator == null) {
			LogContext.warning(
					"Cannot initialize classpath through system properties");
			return;
		}
	...
	public static void initializeJar(String jarName) {
		try {
			...
			if(!jarFile.getClass().getName().equals("java.util.zip.ZipFile")) {
				LogContext.warning(
						"Initialization of classes in jar file not supported for J2ME build");
				return;
			}
	...
	public static void initializeDir(String dirName) {
		...
		if(!file.getClass().getName().equals("java.io.File")) {
			LogContext.warning(
					"Initialization of classes in directory not supported for J2ME build");
			return;
		}
		if(file.isDirectory()) {
	...
	private static void initialize(String prefix, File file) {
		...
		if(file.isDirectory()) {
			final File[] files = file.listFiles();
			final String newPrefix = prefix.length() == 0 ? name
					: prefix + "." + name;
			final int len = files.length;
			for(int i = 0; i < len; ++i) {
