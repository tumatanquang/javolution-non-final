...
public final class MathLib {
	...
	public static final double toDoublePow2(long m, int n) {
		...
		final int bitLength = MathLib.bitLength(m);
	...
	public static final double toDoublePow10(long m, int n) {
		...
		if(n >= 0) { // Positive power.
			...
			final int shift = 31 - MathLib.bitLength(x3); // -1..30
			pow2 -= shift;
		...
		for(;;) {
			// Normalizes x1:x0
			final int shift = 63 - MathLib.bitLength(x1);
			x1 <<= shift;
			x1 |= x0 >>> 63 - shift;
			x0 = x0 << shift & MASK_63;
			pow2 -= shift;
	...
	public static final int floorLog10(double d) {
		final int guess = (int) (LOG2_DIV_LOG10 * MathLib.floorLog2(d));
		final double pow10 = MathLib.toDoublePow10(1, guess);
	...
	static final double _atan(double x) {
		double w, s1, s2, z;
		int ix, hx, id;
	...
	static final double _ieee754_log(double x) {
	...
	static final double _ieee754_exp(double x) { // default IEEE double exp
