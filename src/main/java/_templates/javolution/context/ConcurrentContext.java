 ...
 * };[/code]
 *
 * Here is a concurrent/recursive quick/merge sort using anonymous inner classes:[code]
 * private void quickSort(FastTable<? extends Comparable> table) {
...
public abstract class ConcurrentContext extends Context {
	...
	private static int availableProcessors() {
		final Reflection.Method availableProcessors = Reflection.getInstance()
				.getMethod("java.lang.Runtime.availableProcessors()");
		if(availableProcessors != null) {
			final Integer processors = (Integer) availableProcessors
	 ...
	 */
	public static final Configurable/*<Class<? extends ConcurrentContext>>*/ DEFAULT = new Configurable(
			Default.class) {};
	/**
	...
	public static void setConcurrency(int concurrency) {
		...
		concurrency = MathLib.min(
				((Integer) MAXIMUM_CONCURRENCY.get()).intValue(), concurrency);
	...
	public static void execute(Runnable logic) {
		final ConcurrentContext ctx = (ConcurrentContext) Context
				.getCurrentContext();
	...
	static final class Default extends ConcurrentContext {
		 ...
		 */
		private static final ConcurrentThread[] _Executors = new ConcurrentThread[((Integer) MAXIMUM_CONCURRENCY
				.get()).intValue()];
		static {
			final int len = Default._Executors.length;
			for(int i = 0; i < len; ++i) {
