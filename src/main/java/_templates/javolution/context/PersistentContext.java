...
public class PersistentContext extends Context {
	...
	public static void setCurrentPersistentContext(PersistentContext ctx) {
		...
		synchronized(Reference.INSTANCES) {
			for(FastMap.Entry e = Reference.INSTANCES
					.head(), end = Reference.INSTANCES
							.tail(); (e = (FastMap.Entry) e
									.getNext()) != end;) {
				final Reference reference = (Reference) e.getValue();
	 ...
	 *      =  new FastMap<Unit, FastMap<Unit, Unit>>("UNITS_MULT_LOOKUP").shared();[/code]</p>
	 *
