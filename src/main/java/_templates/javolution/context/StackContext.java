 ...
 *                 if(result.equals(k))
 *                     return StackContext.outerCopy(result); // Exports result.
 *                 k = result;
 *             }
 *         }
 *         finally {
...
public abstract class StackContext extends AllocatorContext {
	...
	private static final class Default extends StackContext {
		...
		protected void deactivate() {
			final FastTable allocators = (FastTable) _activeAllocators.get();
			final int n = allocators.size();
			for(int i = 0; i < n;) {
		...
		protected Allocator getAllocator(ObjectFactory factory) {
			final FastMap factoryToAllocator = (FastMap) _factoryToAllocator
					.get();
			StackAllocator allocator = (StackAllocator) factoryToAllocator
					.get(factory);
			if(allocator == null) {
			...
			if(allocator.user == null) { // Activate.
				...
				final FastTable activeAllocators = (FastTable) _activeAllocators
						.get();
		...
		protected void exitAction() {
			...
			int size = _ownerUsedAllocators.size();
			for(int i = 0; i < size; ++i) {
				final StackAllocator allocator = (StackAllocator) _ownerUsedAllocators
						.get(i);
				allocator.reset();
			}
			_ownerUsedAllocators.clear();
			size = _nonOwnerUsedAllocators.size();
			for(int i = 0; i < size; ++i) {
				final StackAllocator allocator = (StackAllocator) _nonOwnerUsedAllocators
						.get(i);
				allocator.reset();
			}
	...
	private static final class StackAllocator extends Allocator {
		...
		protected Object allocate() {
			...
			final Object obj = _factory.create();
		...
		protected void reset() {
			...
			while(_factory.doCleanup() && queueSize != _queueLimit) {
				final Object obj = queue[queueSize++];
