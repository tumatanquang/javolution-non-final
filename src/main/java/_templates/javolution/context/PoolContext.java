...
public class PoolContext extends AllocatorContext {
	...
	protected void deactivate() {
		final FastTable allocators = (FastTable) ACTIVE_ALLOCATORS.get();
	...
	protected Allocator getAllocator(ObjectFactory factory) {
		...
		PoolAllocator allocator = (PoolAllocator) factoryToAllocator
				.get(factory);
		if(allocator == null) {
		...
		if(allocator.user == null) { // Activate.
			...
			final FastTable activeAllocators = (FastTable) ACTIVE_ALLOCATORS
					.get();
