 ...
 * @see _templates.javolution.util.LocalMap
 */
	...
	public static class Reference
		...
		private Object/*{T}*/ retrieveValue() {
			for(Context ctx = Context
					.getCurrentContext(); ctx != null; ctx = ctx.getOuter()) {
				if(ctx instanceof LocalContext) {
					final LocalContext localContext = (LocalContext) ctx;
					final Object value = localContext._references.get(this);
					if(value != null)
		...
		public void set(Object/*{T}*/ value) {
			final LocalContext ctx = Reference.getLocalContext();
			if(ctx != null) {
				final FastMap references = ctx._references;
