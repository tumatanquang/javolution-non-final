 ...
 * ... // Do something.
 * listFactory.recycle(list); // Optional.[/code]</p>
 *
...
public abstract class ObjectFactory/*<T>*/ {
	 ...
	 */
	public static/*<T>*/ ObjectFactory/*<T>*/ getInstance(
			Class/*<T>*/ forClass) {
		final ObjectFactory factory = (ObjectFactory) Reflection.getInstance()
				.getField(forClass, ObjectFactory.class, false);
	...
	public static/*<T>*/ void setInstance(ObjectFactory/*<T>*/ factory,
			Class/*<T>*/ forClass) {
		Reflection.getInstance().setField(factory, forClass,
				ObjectFactory.class);
	}
	/**
	...
	public final Object/*{T}*/ object() {
		...
		return allocator.user == Thread.currentThread() ? allocator.next()
	...
	public final Allocator/*<T>*/ currentAllocator() {
		...
		allocator = Context.getCurrentContext().getAllocatorContext()
				.getAllocator(this);
	...
	private final ThreadLocal _localAllocator = new ThreadLocal() {
	...
	private static class Generic extends ObjectFactory {
		...
		protected Object create() {
			try {
				...
				throw new Error("Cannot instantiate no-arg constructor for "
						+ _class.getName()
						+ ", the factory should be set explicitly using ObjectFactory.setInstance");
			}
			catch(final IllegalAccessException e) {
				throw new Error("Cannot access no-arg constructor for "
						+ _class.getName()
						+ ", the factory should be set explicitly using ObjectFactory.setInstance");
			}
		}
