...
public class ThreadLocal {
	...
	public ThreadLocal() {}
	public Object get() {
		final FastMap localMap = getLocalMap();
	...
	private static final FastMap getLocalMap() {
		final FastMap localMap = (FastMap) THREAD_TO_LOCAL_MAP
				.get(Thread.currentThread());
		return localMap != null ? localMap : newLocalMap();
	}
	private static final FastMap newLocalMap() {
		// First, do some cleanup (remove dead threads).
		for(FastMap.Entry e = THREAD_TO_LOCAL_MAP
				.head(), end = THREAD_TO_LOCAL_MAP
						.tail(); (e = (FastMap.Entry) e.getNext()) != end;) {
			final Thread thread = (Thread) e.getKey();
			if(!thread.isAlive()) {
				THREAD_TO_LOCAL_MAP.remove(thread);
			}
		}
		final FastMap localMap = new FastMap();
