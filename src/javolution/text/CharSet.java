...
public final class CharSet implements Immutable {
	...
	public static final CharSet WHITESPACES = CharSet.valueOf(
			new char[] {0x9, 0xA, 0xB, 0xC, 0xD, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x1680, 0x180E, 0x2000, 0x2001, 0x2002,
					0x2003, 0x2004, 0x2005, 0x2006, 0x2008, 0x2009, 0x200A, 0x200B, 0x2028, 0x2029, 0x205F, 0x3000});
	/**
	 * Represents spaces characters according to Java
	 * (see {@link Character#isSpaceChar(char)}).
	 */
	public static final CharSet SPACES = CharSet
			.valueOf(new char[] {0x20, 0xA0, 0x1680, 0x180E, 0x2000, 0x2001, 0x2002, 0x2003, 0x2004, 0x2005, 0x2006,
					0x2007, 0x2008, 0x2009, 0x200A, 0x200B, 0x2028, 0x2029, 0x202F, 0x205F, 0x3000});
	/**
	 * Represents ISO control  characters according to Java
	 * (see {@link Character#isISOControl(char)}).
	 */
	public static final CharSet ISO_CONTROLS = CharSet
			.valueOf(new char[] {0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF, 0x10,
	...
	public boolean contains(char c) {
		final int i = c >> 6;
		return i < _mapping.length ? (_mapping[i] & 1L << (c & 63)) != 0 : false;
	}
	/**
	 * Equivalent to
	...
	public CharSet plus(CharSet that) {
		if(that._mapping.length > _mapping.length)
	...
	public CharSet minus(CharSet that) {
		CharSet result = copy();
		for(int i = MathLib.min(_mapping.length, that._mapping.length); --i >= 0;) {
			result._mapping[i] &= ~that._mapping[i];
		}
		return result;
	}
	...
	private CharSet copy() {
		CharSet charSet = new CharSet(new long[_mapping.length]);
