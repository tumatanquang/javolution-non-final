...
public final class Text implements CharSequence, Comparable, XMLSerializable, ValueType, Realtime {
	...
	private static Text valueOf(String str, int start, int end) {
		...
		return newComposite(Text.valueOf(str, start, start + half), Text.valueOf(str, start + half, end));
	}
	/**
	...
	public static Text valueOf(char[] chars, int offset, int length) {
		...
		// Splits on a block boundary.
		int half = length + BLOCK_SIZE >> 1 & BLOCK_MASK;
		return Text.newComposite(Text.valueOf(chars, offset, half), Text.valueOf(chars, offset + half, length - half));
	}
	/**
	...
	static Text valueOf(TextBuilder tb, int start, int end) {
		...
		// Splits on a block boundary.
		int half = length + BLOCK_SIZE >> 1 & BLOCK_MASK;
		return Text.newComposite(Text.valueOf(tb, start, start + half), Text.valueOf(tb, start + half, end));
	}
	/**
	...
	public Text replace(java.lang.CharSequence target, java.lang.CharSequence replacement) {
		...
		return i < 0 ? this : // No target sequence found.
				subtext(0, i).concat(Text.valueOf(replacement))
						.concat(subtext(i + target.length()).replace(target, replacement));
	}
	/**
	...
	public Text replace(CharSet charSet, java.lang.CharSequence replacement) {
		...
		return i < 0 ? this : // No character to replace.
				subtext(0, i).concat(Text.valueOf(replacement)).concat(subtext(i + 1).replace(charSet, replacement));
	}
	/**
	...
	public char charAt(int index) {
		...
		return _data != null ? _data[index]
				: index < _head._count ? _head.charAt(index) : _tail.charAt(index - _head._count);
	}
	/**
	...
	public Text copy() {
		...
		return Text.newComposite(_head.copy(), _tail.copy());
	}
	//////////////////////////////////////////////////////////////////
