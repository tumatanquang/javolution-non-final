...
public abstract class TestContext extends LogContext {
	...
	public static boolean assertArrayEquals(Object[] expected, Object[] actual, CharSequence message) {
		...
		if(expected == null)
			return ctx.doAssert(false,
					message != null ? message : Text.valueOf("Null array expected but found actual array not null"));
		if(actual == null)
			return ctx.doAssert(false,
					message != null ? message : Text.valueOf("Not null array expected but found actual array null"));
		if(expected.length != actual.length)
			return ctx.doAssert(false,
					message != null ? message
							: Text.valueOf("Array of size " + expected.length
									+ " expected but found array of actual size " + actual.length));
		for(int i = 0; i < expected.length; i++) {
			Object e = expected[i];
			Object a = actual[i];
			if(e != null && !e.equals(a) || e != a)
				return ctx.doAssert(false, message != null ? message
						: Text.valueOf("Array element at " + i + ", expected " + e + " but found " + a));
		}
		return ctx.doAssert(true, message);
	}
	/**
	...
	public static boolean assertException(Class exceptionClass, Runnable logic, CharSequence message) {
		...
		if(exception == null)
			return ctx.doAssert(false,
					message != null ? message
							: Text.valueOf("Expected exception instance of ").plus(exceptionClass.getName())
									.plus(" but no exception has been raised"));
		boolean ok = exceptionClass.isInstance(exception);
		message = ok || message != null ? message : // Provides error message if necessary.
				Text.valueOf("Expected instance of ").plus(exceptionClass.getName())
						.plus(" but actual exception is instance of ").plus(exception.getClass().getName());
		return ctx.doAssert(ok, message);
	}
	/**
	 * Convenience method equivalent to
	...
	private static class Default extends TestContext {
		...
		protected void exitAction() {
			...
			logMessage("test", Text.valueOf(
					"SUMMARY - PASSED: " + _passedCount + ", FAILED: " + _failedCount + ", IGNORED: " + _ignoredCount));
		}
		@Override
		protected void doRun(TestSuite testSuite) throws Exception {
			logMessage("test", Text.valueOf("---------------------------------------------------"));
