...
public class Struct {
	...
	private static final Reflection.Method ADDRESS_METHOD = Reflection.getInstance()
	...
	protected <M extends Member> M[] array(M[] arrayMember) {
		...
		if(BOOL.isInstance(arrayMember)) {
			...
			throw new UnsupportedOperationException("Cannot create member elements, the arrayMember should "
	...
	private long readByteBufferLong(int index) {
		...
		if(byteBuffer.order() == ByteOrder.LITTLE_ENDIAN)
			return (readByte(index, byteBuffer) & 0xff) + ((readByte(++index, byteBuffer) & 0xff) << 8)
					+ ((readByte(++index, byteBuffer) & 0xff) << 16) + ((readByte(++index, byteBuffer) & 0xffL) << 24)
					+ ((readByte(++index, byteBuffer) & 0xffL) << 32) + ((readByte(++index, byteBuffer) & 0xffL) << 40)
					+ ((readByte(++index, byteBuffer) & 0xffL) << 48) + ((readByte(++index, byteBuffer) & 0xffL) << 56);
		return ((long) readByte(index, byteBuffer) << 56) + ((readByte(++index, byteBuffer) & 0xffL) << 48)
				+ ((readByte(++index, byteBuffer) & 0xffL) << 40) + ((readByte(++index, byteBuffer) & 0xffL) << 32)
				+ ((readByte(++index, byteBuffer) & 0xffL) << 24) + ((readByte(++index, byteBuffer) & 0xff) << 16)
				+ ((readByte(++index, byteBuffer) & 0xff) << 8) + (readByte(++index, byteBuffer) & 0xffL);
	}
	...
	protected class Member {
		...
		final int get(int wordSize, int word) {
			final int shift = byteOrder() == ByteOrder.BIG_ENDIAN ? (wordSize << 3) - bitIndex() - bitLength()
					: bitIndex();
			word >>= shift;
