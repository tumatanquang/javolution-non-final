...
public class XMLReferenceResolver implements Reusable {
	...
	public boolean writeReference(Object obj, XMLFormat.OutputElement xml) throws XMLStreamException {
		...
		if(id == null) { // New identifier.
			...
			if(_idURI == null) {
				xml.getStreamWriter().writeAttribute(toCsq(_idName), _tmp);
			}
			else {
				xml.getStreamWriter().writeAttribute(toCsq(_idURI), toCsq(_idName), _tmp);
			}
			return false;
		}
		_tmp.clear().append(id.intValue());
		if(_refURI == null) {
			xml._writer.writeAttribute(toCsq(_refName), _tmp);
		}
		else {
			xml._writer.writeAttribute(toCsq(_refURI), toCsq(_refName), _tmp);
		}
		return true;
	}
	...
	public Object readReference(XMLFormat.InputElement xml) throws XMLStreamException {
		CharArray value = xml._reader.getAttributeValue(toCsq(_refURI), toCsq(_refName));
		if(value == null)
			return null;
		int ref = value.toInt();
	...
	public void createReference(Object obj, XMLFormat.InputElement xml) throws XMLStreamException {
		CharArray value = xml._reader.getAttributeValue(toCsq(_idURI), toCsq(_idName));
		if(value == null)
			return;
		int i = value.toInt();
		if(_idToObject.size() != i)
			throw new XMLStreamException(
					"Identifier discontinuity detected " + "(expected " + _idToObject.size() + " found " + i + ")");
		_idToObject.add(obj);
	}
	// Implements Reusable.
	public void reset() {
