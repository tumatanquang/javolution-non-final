...
public abstract class WebServiceClient {
	...
	public void invoke() throws IOException, XMLStreamException {
		try {
			...
			while(xmlIn.hasNext()) {
				if(xmlIn.next() == XMLStreamReader.START_ELEMENT && xmlIn.getLocalName().equals("Body")
		...
		while(xml.hasNext()) {
			...
			case XMLStreamReader.START_ELEMENT:
				...
				for(int i = 0, n = xml.getAttributeCount(); i < n; i++) {
					System.out.println("   Attribute: " + xml.getAttributeLocalName(i) + "("
							+ xml.getAttributeNamespace(i) + "), Value: " + xml.getAttributeValue(i));
				}
				break;
