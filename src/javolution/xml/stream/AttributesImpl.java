...
final class AttributesImpl implements Attributes, Reusable {
	...
	public CharArray getURI(int index) {
		return index >= 0 && index < _length ? _namespaces.getNamespaceURINullAllowed(_prefixes[index]) : null;
	}
	// Implements Attributes.
	...
	public String toString() {
		...
		for(int i = 0; i < _length;) {
			text = text.concat(Text.valueOf(_qNames[i]).concat(equ).concat(Text.valueOf(_values[i])));
			if(++i != _length) {
