...
public final class XMLStreamReaderImpl implements XMLStreamReader, Reusable {
	 ...
	 */
	static final String[] NAMES_OF_EVENTS = new String[] {"UNDEFINED", "START_ELEMENT", "END_ELEMENT",
			"PROCESSING_INSTRUCTIONS", "CHARACTERS", "COMMENT", "SPACE", "START_DOCUMENT", "END_DOCUMENT",
			"ENTITY_REFERENCE", "ATTRIBUTE", "DTD", "CDATA", "NAMESPACE", "NOTATION_DECLARATION", "ENTITY_DECLARATION"};
	/**
	...
	private static boolean isUTF8(Object encoding) {
		return encoding.equals("utf-8") || encoding.equals("UTF-8") || encoding.equals("ASCII")
				|| encoding.equals("utf8") || encoding.equals("UTF8");
	}
	/**
	...
	public void setInput(Reader reader) throws XMLStreamException {
		...
		try { // Reads prolog (if there)
			int readCount = reader.read(_readBuffer, _startOffset, _readBuffer.length - _startOffset);
			_readCount = readCount >= 0 ? readCount + _startOffset : _startOffset;
			if(_readCount >= 5 && _readBuffer[0] == '<' && _readBuffer[1] == '?' && _readBuffer[2] == 'x'
					&& _readBuffer[3] == 'm' && _readBuffer[4] == 'l' && _readBuffer[5] == ' ') { // Prolog detected.
	...
	public CharArray getQName() {
		if(_eventType != XMLStreamConstants.START_ELEMENT && _eventType != XMLStreamConstants.END_ELEMENT)
		...
		while(true) {
			...
			case STATE_CDATA:
				while(true) { // Reads CDATA all at once.
					if(c == '>' && _index - _start >= 3 && _data[_index - 2] == ']' && _data[_index - 3] == ']') {
						_index -= 3;
			...
			case STATE_MARKUP: // Starts with '<'
				...
				else if(_index - _start == 4 && _data[_start + 1] == '!' && _data[_start + 2] == '-'
						&& _data[_start + 3] == '-') {
				...
				else if(_index - _start == 9 && _data[_start + 1] == '!' && _data[_start + 2] == '['
						&& _data[_start + 3] == 'C' && _data[_start + 4] == 'D' && _data[_start + 5] == 'A'
						&& _data[_start + 6] == 'T' && _data[_start + 7] == 'A' && _data[_start + 8] == '[') {
					_start = _index = _index - 9; // Do not keep <![CDATA[
					_state = STATE_CDATA;
				}
				else if(_index - _start == 9 && _data[_start + 1] == '!' && _data[_start + 2] == 'D'
						&& _data[_start + 3] == 'O' && _data[_start + 4] == 'C' && _data[_start + 5] == 'T'
						&& _data[_start + 6] == 'Y' && _data[_start + 7] == 'P' && _data[_start + 8] == 'E') {
	...
	private void reloadBuffer() throws XMLStreamException {
		...
		try {
			...
			if(_readCount <= 0 && (_depth != 0 || _state != STATE_CHARACTERS))
				throw new XMLStreamException("Unexpected end of document", _location);
		}
		catch(IOException e) {
			throw new XMLStreamException(e);
		}
	...
	private char replaceEntity() throws XMLStreamException {
		if(_state == STATE_COMMENT || _state == STATE_PI || _state == STATE_CDATA)
			return '&'; // (&2.4)
	...
	private void processAttribute() throws XMLStreamException {
		if(_attrPrefixSep < 0) { // No prefix.
			if(isXMLNS(_attrQName)) { // Sets default namespace.
				...
				_attributes.addAttribute(_attrQName, null, _attrQName, _attrValue);
			}
		}
		else { // Prefix.
			...
			CharArray localName = newSeq(_attrPrefixSep + 1, offset + length - _attrPrefixSep - 1);
			if(isXMLNS(prefix)) { // Namespace association.
				_namespaces.setPrefix(localName, _attrValue);
			}
			else {
				_attributes.addAttribute(localName, prefix, _attrQName, _attrValue);
			}
		}
	}
	private static boolean isXMLNS(CharArray chars) {
		return chars.length() == 5 && chars.charAt(0) == 'x' && chars.charAt(1) == 'm' && chars.charAt(2) == 'l'
				&& chars.charAt(3) == 'n' && chars.charAt(4) == 's';
	}
	private void processEndTag() throws XMLStreamException {
		if(!_qName.equals(_elemStack[_depth]))
			throw new XMLStreamException("Unexpected end tag for " + _qName, _location);
	}
	...
	private CharArray newSeq(int offset, int length) {
		CharArray seq = _seqsIndex < _seqsCapacity ? _seqs[_seqsIndex++] : newSeq2();
	...
	public void require(int type, CharSequence namespaceURI, CharSequence localName) throws XMLStreamException {
		if(_eventType != type)
			throw new XMLStreamException(
					"Expected event: " + NAMES_OF_EVENTS[type] + ", found event: " + NAMES_OF_EVENTS[_eventType]);
		...
		while(eventType != XMLStreamConstants.END_ELEMENT) {
			if(eventType == XMLStreamConstants.CHARACTERS) {
				if(text == null) {
					...
					text.setArray(_data, text.offset(), text.length() + getText().length());
				}
			}
	...
	public CharArray getAttributeValue(CharSequence uri, CharSequence localName) {
		if(_eventType != XMLStreamConstants.START_ELEMENT)
			throw illegalState("Not a start element");
		return uri == null ? _attributes.getValue(localName) : _attributes.getValue(uri, localName);
	}
	...
	public CharArray getLocalName() {
		...
		if(_prefixSep < 0)
			return _qName;
		CharArray localName = newSeq(_prefixSep + 1, _qName.offset() + _qName.length() - _prefixSep - 1);
		return localName;
	}
	...
	public CharArray getText() {
		if(_eventType != XMLStreamConstants.CHARACTERS && _eventType != XMLStreamConstants.COMMENT
	...
	public boolean hasName() {
		return _eventType == XMLStreamConstants.START_ELEMENT || _eventType == XMLStreamConstants.END_ELEMENT;
	}
	public boolean hasNext() throws XMLStreamException {
		return _eventType != XMLStreamConstants.END_DOCUMENT;
	}
	public boolean hasText() {
		return (_eventType == XMLStreamConstants.CHARACTERS || _eventType == XMLStreamConstants.COMMENT
	...
	public int nextTag() throws XMLStreamException {
		int eventType = next();
		while(eventType == XMLStreamConstants.COMMENT || eventType == XMLStreamConstants.PROCESSING_INSTRUCTION
				|| eventType == XMLStreamConstants.DTD
				|| eventType == XMLStreamConstants.CHARACTERS && isWhiteSpace()) {
			eventType = next();
		}
		if(eventType != XMLStreamConstants.START_ELEMENT && eventType != XMLStreamConstants.END_ELEMENT)
			throw new XMLStreamException("Tag expected (but found " + NAMES_OF_EVENTS[_eventType] + ")");
		return eventType;
	}
	private IllegalStateException illegalState(String msg) {
		return new IllegalStateException(msg + " (" + NAMES_OF_EVENTS[_eventType] + ")");
	}
