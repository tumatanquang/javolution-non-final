...
public final class XMLStreamWriterImpl implements XMLStreamWriter, Reusable {
	...
	// Implements XMLStreamWriter interface.
	public void writeStartElement(CharSequence prefix, CharSequence localName, CharSequence namespaceURI)
	...
	// Implements XMLStreamWriter interface.
	public void writeEmptyElement(CharSequence prefix, CharSequence localName, CharSequence namespaceURI)
			throws XMLStreamException {
	...
	// Implements XMLStreamWriter interface.
	public void writeAttribute(CharSequence namespaceURI, CharSequence localName, CharSequence value)
			throws XMLStreamException {
		if(localName == null)
			throw new XMLStreamException("Local name cannot be null");
		if(value == null)
	...
	// Implements XMLStreamWriter interface.
	public void writeAttribute(CharSequence prefix, CharSequence namespaceURI, CharSequence localName,
	...
	public void writeNamespace(CharSequence prefix, CharSequence namespaceURI) throws XMLStreamException {
		if(prefix == null || prefix.length() == 0 || _namespaces._xmlns.equals(prefix)) {
		...
		_namespaces.setPrefix(prefix, namespaceURI == null ? _namespaces._nullNsURI : namespaceURI, true);
	}
	// Implements XMLStreamWriter interface.
	...
	public void setPrefix(CharSequence prefix, CharSequence uri) throws XMLStreamException {
		_namespaces.setPrefix(prefix, uri == null ? _namespaces._nullNsURI : uri, false);
	}
	// Implements XMLStreamWriter interface.
	...
	private void writeNewElement(CharSequence prefix, CharSequence localName, CharSequence namespaceURI)
			throws XMLStreamException {
		...
		if(namespaceURI != null && !_namespaces._defaultNamespace.equals(namespaceURI)) {
			if(_isRepairingNamespaces) { // Repairs prefix.
				...
				if(prefix == null)
					throw new XMLStreamException(
							"URI: " + namespaceURI + " not bound and repairing namespaces disabled");
			}
			if(prefix.length() > 0) {
				qName.append(prefix);
				qName.append(':');
	...
	private void writeAttributeOrNamespace(CharSequence prefix, CharSequence namespaceURI, CharSequence localName,
			CharSequence value) throws XMLStreamException {
		if(!_isElementOpen)
			throw new IllegalStateException("No open start element");
		write(' ');
		...
		for(int i = i0; i < i2; i++) {
			if(_isRepairingNamespaces && i < i1 && !_namespaces._prefixesWritten[i]
				...
				if(_isRepairingNamespaces) {
					CharArray prefix = _namespaces.getPrefix(_namespaces._namespaces[i], i);
				...
				if(_namespaces._prefixes[i].length() == 0) { // Default namespace.
					writeAttributeOrNamespace(null, null, _namespaces._xmlns, _namespaces._namespaces[i]);
				}
				else {
					writeAttributeOrNamespace(_namespaces._xmlns, _namespaces._xmlnsURI, _namespaces._prefixes[i],
	...
	private CharSequence getRepairedPrefix(CharSequence prefix, CharSequence namespaceURI) throws XMLStreamException {
		...
		if(prefixForURI != null && (prefix == null || prefixForURI.equals(prefix)))
			return prefixForURI; // No repair needed.
		if(prefix == null || prefix.length() == 0) { // Creates new prefix.
			prefix = _autoPrefix.clear().append(_repairingPrefix).append(_autoNSCount++);
		}
	...
	private final void write(Object csq, int start, int length, boolean escapeMarkup) throws XMLStreamException {
		if(_index + length <= BUFFER_LENGTH) { // Enough buffer space.
			if(csq instanceof String) {
				...
				((javolution.text.Text) csq).getChars(start, start + length, _buffer, _index);
			}
			else if(csq instanceof javolution.text.TextBuilder) {
				((javolution.text.TextBuilder) csq).getChars(start, start + length, _buffer, _index);
			}
			else if(csq instanceof javolution.text.CharArray) {
				((javolution.text.CharArray) csq).getChars(start, start + length, _buffer, _index);
			}
			else {
				getChars((CharSequence) csq, start, start + length, _buffer, _index);
			}
			if(escapeMarkup) {
	...
	private boolean isEscaped(char c) {
		return c < ' ' && _isAttributeValue || c == '"' && _isAttributeValue || c == '<' || c == '>' || c == '&';
	}
