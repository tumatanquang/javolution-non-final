...
final class EntitiesImpl implements Reusable {
	...
	public int replaceEntity(char[] buffer, int start, int length) throws XMLStreamException {
		...
		if(buffer[start + 1] == 'a' && buffer[start + 2] == 'p' && buffer[start + 3] == 'o' && buffer[start + 4] == 's'
				&& buffer[start + 5] == ';') {
			buffer[start] = '\'';
			return 1;
		}
		if(buffer[start + 1] == 'q' && buffer[start + 2] == 'u' && buffer[start + 3] == 'o' && buffer[start + 4] == 't'
				&& buffer[start + 5] == ';') {
			buffer[start] = '"';
			return 1;
		}
		if(buffer[start + 1] == 'a' && buffer[start + 2] == 'm' && buffer[start + 3] == 'p'
				&& buffer[start + 4] == ';') {
			buffer[start] = '&';
			return 1;
		}
		...
		CharSequence replacementText = _entitiesMapping != null ? (CharSequence) _entitiesMapping.get(_tmp) : null;
