...
public class XMLBinding implements Reusable, XMLSerializable {
	...
	protected Class readClass(XMLStreamReader reader, boolean useAttributes) throws XMLStreamException {
		...
		if(useAttributes) {
			...
			classQName = QName.valueOf(
					reader.getAttributeValue(_classAttribute.getNamespaceURI(), _classAttribute.getLocalName()));
			if(classQName == null)
	...
	protected void writeClass(Class cls, XMLStreamWriter writer, boolean useAttributes) throws XMLStreamException {
		...
		if(useAttributes) {
			...
			if(_classAttribute.getNamespaceURI() == null) {
				writer.writeAttribute(_classAttribute.getLocalName(), QName.j2meToCharSeq(name));
			}
			else {
				writer.writeAttribute(_classAttribute.getNamespaceURI(), _classAttribute.getLocalName(),
		...
		public Object newInstance(Class cls, javolution.xml.XMLFormat.InputElement xml) throws XMLStreamException {
			...
			if(!format.isParsingSupported())
				throw new XMLStreamException(
						"No XMLFormat or TextFormat (with parsing supported) for instances of " + cls);
	 ...
	 * default constructor.
	 */
	static final XMLFormat COLLECTION_XML = new XMLFormat(java.util.Collection.class) {
		@Override
		public void read(InputElement xml, Object obj) throws XMLStreamException {
