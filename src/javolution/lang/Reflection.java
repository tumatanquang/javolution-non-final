			...
			synchronized(forClass) { // We don't want to attach simultaneously to the same class.
				...
				if(typeToField != null && typeToField.containsKey(type))
					throw new IllegalArgumentException(
							"Field of type " + type + " already attached to class " + forClass);
				if(typeToField == null) {
		...
		private static class DefaultConstructor extends BaseConstructor {
			...
			public Object allocate(Object[] args) {
				try {
					...
					throw new RuntimeException("Default constructor instantiation error for " + _class.getName() + " ("
							+ e.getMessage() + ")");
				}
				catch(IllegalAccessException e) {
					throw new RuntimeException("Default constructor illegal access error for " + _class.getName() + " ("
							+ e.getMessage() + ")");
				}
			}
			@Override
		...
		private final class ReflectConstructor extends BaseConstructor {
			...
			public Object allocate(Object[] args) {
				try {
					...
					throw new RuntimeException("Invocation exception  for " + _signature + " constructor",
							e.getCause());
				}
			}
			@Override
		...
		private final class ReflectMethod extends BaseMethod {
			...
			public Object execute(Object that, Object[] args) {
				try {
					...
					if(e.getCause() instanceof RuntimeException)
						throw (RuntimeException) e.getCause();
					throw new RuntimeException("Invocation exception for " + _signature + " method", e.getCause());
				}
			}
			@Override
			public String toString() {
