...
public abstract class ObjectFactory<T> {
	...
	public static <T> ObjectFactory<T> getInstance(Class<T> forClass) {
		ObjectFactory factory = Reflection.getInstance().getField(forClass, ObjectFactory.class, false);
	 ...
	 */
	public static <T> void setInstance(ObjectFactory<T> factory, Class<T> forClass) {
	...
	private static class Generic extends ObjectFactory {
		...
		protected Object create() {
			try {
				...
				throw new Error("Cannot instantiate no-arg constructor for " + _class.getName()
						+ ", the factory should be set explicitly using ObjectFactory.setInstance");
			}
			catch(IllegalAccessException e) {
				throw new Error("Cannot access no-arg constructor for " + _class.getName()
						+ ", the factory should be set explicitly using ObjectFactory.setInstance");
			}
		}
	}
